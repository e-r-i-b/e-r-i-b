...
<c:set var="rurPaymentDocument" value="${form.response.document.newRurPaymentDocument}"/>
<tiles:insert page="fields-table.jsp" flush="false">
    <tiles:put name="data">
        ...
        <c:if test="${not empty rurPaymentDocument.commission}">
            ...
            <td>Комиcсия</td> <td><c:out value="${rurPaymentDocument.commission.amount}"/> <c:out value="${phiz:normalizeCurrencyCode(rurPaymentDocument.commission.currency.code)}"/></td>
        </c:if>
        <c:if test="${not empty rurPaymentDocument.operationCode}">
            <tiles:insert page="field.jsp" flush="false">
                <tiles:put name="field" beanName="rurPaymentDocument" beanProperty="operationCode"/>
            </tiles:insert>
        </c:if>
