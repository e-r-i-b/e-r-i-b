...
/
update SRB_IKFL.LOAN_LINKS set SHOW_IN_SOCIAL = SHOW_IN_MOBILE
/
...
/
update SRB_IKFL.IMACCOUNT_LINKS set SHOW_IN_SOCIAL = SHOW_IN_MOBILE
/
...
alter table SRB_IKFL.DEPOSITGLOBALS add PERCENT_RATES_TRANSFORMATION CLOB
/
...
CREATE OR REPLACE PROCEDURE SRB_IKFL.updateExceptionInformation(exceptionKind in varchar, exceptionHash in varchar, exceptionOperation in varchar, exceptionApplication in varchar, exceptionDetail in clob, exceptionSystem in varchar, exceptionErrorCode in varchar) as
...
alter table SRB_IKFL.CURBANKS rename to FORDEL$CURBANKS
/
...
/
alter index SRB_IKFL.I_INCOGNITO_PHONES rename to IDX_PH_INCOGNITO;
/
...
alter table SRB_IKFL.RECENTLY_FILLED_FIELD_DATA rename to FORDEL$RECENTLY_F_F_DATA
/ 
...
alter table SRB_IKFL.CONTACTS rename to FORDEL$CONTACTS
/
alter table SRB_IKFL.CONTACTS_SYNC rename to FORDEL$CONTACTS_SYNC
/
...
alter table SRB_IKFL.ERMB_PROFILE add GRACE_PERIOD_DATE timestamp
/
update SRB_IKFL.ERMB_PROFILE
   set GRACE_PERIOD_DATE = add_months (CONNECTION_DATE, (select GRACE_PERIOD from SRB_IKFL.ERMB_TARIF where ID = ERMB_TARIF_ID) )
/   
alter table SRB_IKFL.ERMB_PROFILE modify GRACE_PERIOD_DATE not null
/

-- Номер ревизии: 69906
-- Комментарий: BUG078978: Добавить группу настроек по новому каналу в блок "Настройка видимости в каналах" 
alter table SRB_IKFL.PAYMENT_SERVICES add SHOW_IN_SOCIAL char(1)
/
update SRB_IKFL.PAYMENT_SERVICES set SHOW_IN_SOCIAL = '1'
/
alter table SRB_IKFL.PAYMENT_SERVICES modify SHOW_IN_SOCIAL default '1' not null 
/

-- Номер ревизии: 69974
-- Комментарий: ENH078242: [ISUP] Полный просмотр таблицы CardOperationClaim
create index SRB_IKFL.I_CARD_OPERATION_CLAIMS_STATUS on SRB_IKFL.CARD_OPERATION_CLAIMS(
    decode(STATUS, 'TIMEOUT', 1, 'PROCESSING', 2, 'WAITING', 3, 'AUTO_WAITING', 4)
) tablespace INDX parallel 32 nologging
/
alter index SRB_IKFL.I_CARD_OPERATION_CLAIMS_STATUS noparallel logging
/

-- Номер ревизии: 70165
-- Комментарий: BUG077756: Ошибка при поиске расширенной заявки на кредит 
update SRB_IKFL.BUSINESS_DOCUMENTS
...
alter index SRB_IKFL.DXFK_LOAN_OFFER rename to FORDEL$DXFK_LOAN_OFFER
/

create index SRB_IKFL.DXFK_LOAN_OFFER on SRB_IKFL.LOAN_OFFER (
...
/
alter index SRB_IKFL.DXFK_LOAN_OFFER noparallel logging
/

drop index SRB_IKFL.FORDEL$DXFK_LOAN_OFFER
/
alter table SRB_IKFL.LOAN_OFFER drop ( LOGIN_ID, DEPARTMENT_ID)  
/

-- Номер ревизии: 66619
-- Комментарий: Справочник тарифных планов
update SRB_IKFL.RATE set TARIF_PLAN_CODE = '1' where TARIF_PLAN_CODE = 'PREMIER'
/
update SRB_IKFL.RATE set TARIF_PLAN_CODE = '2' where TARIF_PLAN_CODE = 'GOLD'
/
update SRB_IKFL.RATE set TARIF_PLAN_CODE = '3' where TARIF_PLAN_CODE = 'FIRST'
/
update SRB_IKFL.RATE set TARIF_PLAN_CODE = null where TARIF_PLAN_CODE = 'UNKNOWN'
/

-- Номер ревизии: 70427
-- Комментарий: BUG081606: [БКИ] Ошибка при сохранении изменения в настройках времени отправки запроса
delete from SRB_IKFL.PROPERTIES
where 
...
insert into SRB_IKFL.ADAPTERS(ID, UUID, NAME, ADAPTER_TYPE, LISTENER_URL) 
        values(SRB_IKFL.S_ADAPTERS.nextval, 'phiz-mbk', 'МБК', 'ESB', '')
/

--Скрипт для установки параметров отображения комиссий, с учетом доработок.
update 
    SRB_IKFL.COMMISSIONS_SETTINGS
set 
...
update SRB_IKFL.DEPARTMENTS set TIME_ZONE = TIME_ZONE + 1
/

delete from SRB_IKFL.PAYMENTFORMS where NAME in (
...
delete from SRB_IKFL.CALENDARPAYMENTFORM where PAYMENTFORM_ID in (
	select ID from SRB_IKFL.PAYMENTFORMS where NAME in (
		'CardReplenishmentPayment',
		'PurchaseCurrencyPayment',
		'SaleCurrencyPayment',
		'TaxPayment',
...
alter table SRB_IKFL.PHONES modify REGISTRATION_DATE null
/ 

INSERT INTO SRB_IKFL.TARIF_PLAN_CONFIGS(ID, TARIF_PLAN_CODE, NEED_SHOW_STANDART_RATE, PRIVILEGED_RATE_MESSAGE, NAME, DATE_BEGIN, DATE_END, STATE)
...
delete from SRB_IKFL.QRTZ_CRON_TRIGGERS
where trigger_name in (
'DocumentConverterJobTrigger',
'UpdateIncognitoPhonesJob')
/
delete from SRB_IKFL.QRTZ_SIMPLE_TRIGGERS
where trigger_name in (
'DocumentConverterJobTrigger',
'UpdateIncognitoPhonesJob')
/
delete from SRB_IKFL.qrtz_triggers
where trigger_name in (
'DocumentConverterJobTrigger',
'UpdateIncognitoPhonesJob')
/
delete from SRB_IKFL.QRTZ_JOB_DETAILS
where job_name in (
