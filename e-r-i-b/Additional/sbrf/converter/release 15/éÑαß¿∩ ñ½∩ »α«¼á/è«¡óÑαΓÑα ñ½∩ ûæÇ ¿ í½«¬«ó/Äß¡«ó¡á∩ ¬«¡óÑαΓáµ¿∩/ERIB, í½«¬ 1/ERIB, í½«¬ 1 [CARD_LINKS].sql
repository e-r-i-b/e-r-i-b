...
alter table SRB_IKFL.FORDEL$CARD_LINKS rename constraint PK_CARD_LINKS to FORDEL$CARD_LINKS_C_1;
alter table SRB_IKFL.FORDEL$CARD_LINKS rename constraint FK_CARD_LIN_FK_CARDLI_LOGINS to FORDEL$CARD_LINKS_C_5;

alter table SRB_IKFL.ERMB_PROFILE drop constraint FK_FOREG_PRODUCT;
alter table SRB_IKFL.SENDED_ABSTRACTS drop constraint FK_SENDED_A_TO_CARD_LIN;
alter table SRB_IKFL.STORED_CARD drop constraint FK_STORED_C_TO_LINK_REF;

alter index SRB_IKFL.DXFK_CARDLINKS_TO_LOGINS rename to FORDEL$CARD_LINKS_IDX_2;
alter index SRB_IKFL.UNIQ_CARD_NUMBER rename to FORDEL$CARD_LINKS_IDX_3;
alter index SRB_IKFL.PK_CARD_LINKS rename to FORDEL$CARD_LINKS_IDX_4;

alter session force parallel ddl parallel 32;

create table SRB_IKFL.CARD_LINKS tablespace USERS nologging as 
...
alter table SRB_IKFL.CARD_LINKS add CONTRACT_NUMBER varchar2(64);
...
alter table SRB_IKFL.CARD_LINKS add constraint PK_CARD_LINKS primary key (ID) using index SRB_IKFL.PK_CARD_LINKS enable novalidate;
