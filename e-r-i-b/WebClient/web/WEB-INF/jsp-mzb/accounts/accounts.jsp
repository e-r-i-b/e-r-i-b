		...
		for (var i = 1; i < table.rows.length; i++)
		{
			...
			if (checkHide.innerHTML == "true")
			{
				if(view)
				{
					...
					lineNumber++;
					table.rows[i].className = "ListLine"+lineNumber%2;
				}
				else
				{
...
<tr>
<td height="100%">
<div class="MaxSize" id="accountDiv" style="overflow:auto;">
<table class="MaxSize"><tr><td valign="top">
<% int lineNumber = 0;%>
<!--список счетов -->
					...
					<td class="ListItem" nowrap="true">
						<c:choose>
							<c:when test="${card.cardType=='overdraft'}">
								<div style="text-align:center;" class=MaxSize>
									<phiz:link action="/private/cards/info">
                                       <phiz:param name="cardId" value="${cardlink.id}"/>
...
<c:if test="${accountsCount+cardsCount+depositsCount>0}">
	<input name="ViewAll" id="ViewAll" type="checkbox"
		style="border:none" onclick="viewAll();">Показывать неактивные счета/вклады/карты</input>
</c:if>
<br>
</td>
</tr>
...
<tr>
	<td>
		<table cellspacing="0" cellpadding="0" class="MaxSize" id="systemsNewsContent">
			<tr>
				<td>
					<!-- краткие новости -->
