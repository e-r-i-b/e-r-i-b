...
<tiles:insert definition="check">
    ...
    <div id="checkId" class="mainDiv">
        ...
        ПО УСЛУГЕ АВТОПЛАТЕЖ<c:if test="${not empty payment.autopayNumber}"><br/>НОМЕР АВТОПЛАТЕЖА:<c:out value="${payment.autopayNumber}"/></c:if><br/><br/>БЕЗНАЛИЧНАЯ ОПЛАТА УСЛУГ</div>
        <br/>
        ...
        <c:choose>
            <c:when test="${not empty payment.receiverCard &&
                ...
                <div class="checkSize">СЧЕТ: ${phiz:getCutCardNumber(payment.receiverCard)} </div>
            </c:when>
        ...
        <div id="stamp" class="stamp title">
            ...
            <br/>
            <img width="137px" src="${imagePath}/stampExecuted_blue.gif">
        </div>
