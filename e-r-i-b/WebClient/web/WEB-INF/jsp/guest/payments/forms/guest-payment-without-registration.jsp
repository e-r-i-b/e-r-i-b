                    ...
                    <div class="notice ">
                        <c:choose>
                            <c:when test="${isLoanCardClaim && hasPhoneInMobileBank && not empty form.document.offerId}">
                                <div class="noticeTitle">Отлично! Заявка успешно отправлена в Банк</div>
                                Карта будет выпущена и доставлена в выбранное отделение.
                                Для получения потребуется паспорт или другой документ, удостоверяющий личность.
                                Уточнить, доставлена ли карта, можно по телефону отделения ${phiz:getDepartmentPhoneNumber(form.document)}
                            </c:when>
                            <c:otherwise>
                                <div class="noticeTitle">Спасибо! Мы приняли вашу заявку</div>
                                Наш сотрудник свяжется с вами в ближайшее время и проконсультирует по дальнейшим шагам
                            </c:otherwise>
                        </c:choose>
                    </div>
                </tiles:put>
