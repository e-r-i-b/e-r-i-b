...
<tiles:insert definition="printDoc">
	...
	<c:forEach items="${PrintAccountAbstractForm.accountAbstract}" var="listElement">
		...
		<br/>
		<c:choose>
		<c:when test="${phiz:size(resourceAbstract.transactions)==0}">
	...
	<c:if test="${cardlink.main}">
        ...
        <c:if test="${phiz:size(abstr.unsettledOperations)==0}">
          <span class="messageTab">
             Нет операций, по которым ожидается списание.
          </span>
        </c:if>

     	<c:if test="${cardlink.card.cardType=='overdraft'}">
	...
	<c:forEach items="${PrintAccountAbstractForm.depositAbstract}" var="listElement">
		...
		<span>ВЫПИСКА ПО СЧЕТУ № </span><b>
		<c:out value="${depositLink.depositInfo.number}"/>
	</b>
		<nobr>
			На дату
			<b>
		...
		<br/>
		Валюта счета:
		<c:out value="${depositLink.depositInfo.currency.code}"/>
		<br/>
		<br/>
		Входящий остаток:&nbsp;
		<bean:write name="resourceAbstract" property="openingBalance.decimal" format="0.00"/>
