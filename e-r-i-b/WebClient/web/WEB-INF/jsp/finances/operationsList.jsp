...
<html:form action="/private/async/operations/list" >
    ...
    <c:set var="showCreditCard" value="${form.filters['showCreditCards']}"/>
                    ...
                    <c:choose>
                        <c:when test="${size > 0}">
                            <c:forEach items="${operations}" var="operationDescription" end="${resOnPage - 1}">
                                <c:set var="editFunction" value="editOperation.setListOperationData(this, '${fromDate}','${toDate}', '${typeDate}', ${searchPage}, ${resOnPage}, ${showCreditCard}, true, '${openPageDate}', '${income}', '${onlyAvailableCategories}'); editOperation.open(${operationDescription.id});"/>
            ...
            <script type="text/javascript">
                function changePage(elem, paginationType)
                {
                    ...
                    var params = {
                        ...
                        toDate: '${toDate}',
                        typeDate: '${typeDate}',
                        showCreditCards: ${showCreditCard},
                        ...
                        editOperation.operations.push({
                            ...
                            categories: [
                               <c:forEach var="category" items="${operationDescription.availableCategories}" varStatus="i">
                                   ...
                                   { id: ${category.id}, title: "${fn:escapeXml(category.name)}" }
                               </c:forEach>
                            ],
