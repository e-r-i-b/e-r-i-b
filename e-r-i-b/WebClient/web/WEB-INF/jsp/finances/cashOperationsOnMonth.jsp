...
<c:forEach items="${form.monthList}" var="item" varStatus="i">
    ...
    <div>
        <tiles:insert definition="cashDayTemplate" flush="false">
            <tiles:put name="id" value="day_${day}"/>
            <tiles:put name="cashIncome" value="${item.cashIncome}"/>
            <tiles:put name="cashOutcome" value="${item.cashOutcome}"/>
            <tiles:put name="cashlessIncome" value="${item.cashlessIncome}"/>
            <tiles:put name="cashlessOutcome" value="${item.cashlessOutcome}"/>
...
<div id="Operations${id}" class="operationsListBlock" style="display:none;">
    ...
    <div class="filter triggerFilter">
        <div class="greenSelector active" id="cash_${id}" onclick="financesUtils.changeSearchTypeValue('${id}', 'cash', '', '${date}', ${showCreditCard}, '${openPageDate}', getElementValue('filter(showOtherAccounts)')); return false;">
           <span>Все операции</span>
        </div>
        <div class="greenSelector transparent" id="cash_1${id}" onclick="financesUtils.changeSearchTypeValue('${id}', 'cash', '1', '${date}', ${showCreditCard}, '${openPageDate}', getElementValue('filter(showOtherAccounts)')); return false;">
           <span>Операции с наличными</span>
        </div>
        <div class="greenSelector transparent" id="cash_0${id}" onclick="financesUtils.changeSearchTypeValue('${id}', 'cash', '0', '${date}', ${showCreditCard}, '${openPageDate}', getElementValue('filter(showOtherAccounts)')); return false;">
...
<script type="text/javascript">
    doOnLoad(function()
    {
        financesUtils.showOperations('Operations${id}', '${date}', ${showCreditCard}, '${openPageDate}');
    });
</script>