...
<html:form action="/private/accounts/info" onsubmit="return setEmptyAction(event)">
    ...
    <c:set var="isERMBConnectedPerson" value="${phiz:isERMBConnectedPerson()}" scope="request"/>

    <c:set var="account" value="${accountLink.account}"/>
    <c:set var="isLock" value="${account.accountState!='OPENED'}"/>
    <c:set var="accountBalans" value="${fn:substring(account.number, 0,5)}"/>
    <c:set var="accountInfoUrl" value="${phiz:calculateActionURL(pageContext,'/private/accounts/info.do?id=')}"/>
                    ...
                    <tiles:insert definition="roundBorderWithoutTop" flush="false">
                        ...
                        <tiles:put name="data">
                            ...
                            <tiles:insert definition="financeContainer" flush="false">
                                <jsp:include page="accountInfoData.jsp"/>
                            </tiles:insert>
                        </tiles:put>
                    </tiles:insert>
                </tiles:put>
            ...
            <tiles:insert definition="accountInfo">
                ...
                <c:if test="${not empty form.accountLink}">
                    ...
                    <tiles:put name="breadcrumbs">
                        ...
                        <tiles:insert definition="breadcrumbsLink" flush="false">
                            <tiles:put name="name"><bean:write name="accountLink" property="name"/> ${phiz:getFormattedAccountNumber(account.number)}</tiles:put>
                            <tiles:put name="last" value="true"/>
                        </tiles:insert>
                    </tiles:put>
