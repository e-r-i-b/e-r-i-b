...
<c:forEach var="el" items="${result}" varStatus="stat">
    ...
    <c:set var="nameWithoutQuotes">${phiz:replaceSymbol(providerName)}</c:set>
    <c:set var="firstLetter" value="${fn:substring(nameWithoutQuotes, 0, 1)}"/>
    <c:if test="${resultType == 'payment'}">
        <c:set var="category"        value="${el[11]}"/>
    </c:if>
    <c:if test="${not (stringCount + providerCountInBlock/2 == 4 || (stringCount + providerCountInBlock/2 == 3.5 && fn:toUpperCase(firstLetter) != currLetter))}">
        <c:if test="${providerCountInBlock % 2 == 0}">
            <div class="clear"></div>
        </c:if>
        
        <c:if test="${fn:toUpperCase(firstLetter) != currLetter}">
            ...
            <c:set var="currLetter" value="${fn:toUpperCase(firstLetter)}"/>
            <div class="clear"></div>
            <div class="titleRegion">
                <span class="bold">${currLetter}</span>
            </div>
