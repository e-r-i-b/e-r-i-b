...
<%--
  Шаблон для подтверждения операции SMS паролем
    ...
    data                - данные отображаемые пользователю (сюда забиваем данные для сверки с SMS паролем)
    ...
    guest               - признак гостевой сессии
    documentShortNumber - короткий номер заявки (для гостевой сессии)
--%>
...
<h2>
    <c:choose>
        ...
        <c:when test="${confirmableObject == 'creditOffertText'}">
            Просмотреть договор с индивидуальными условиями кредитования
        </c:when>
        ...
        <c:when test="${confirmableObject == 'incognitoSetting'}">
            Подтверждение настроек приватности
        </c:when>
        ...
        <c:when test="${confirmableObject == 'EarlyLoanRepaymentClaimPartial'}">
            Подтверждение частичного досрочного погашения
        </c:when>
            ...
            <div class="singleRowPasswordBlock">
                <c:if test="${not needNewPassword}">
                    <div class="singleRowPassword">
                        <span class="fieldTitle" style="color: ${textColor}" id="fieldTitle">Введите SMS-пароль</span>
    ...
    <c:otherwise>
        ...
        <script type="text/javascript">
            function validatePasswordField()
            {
                ...
                {
                    addError("Введите пароль, полученный через sms.", 'warningMessages');
