...
public class CurrencyHelper
{
	...
	public CurrencyImpl getCurrencyById(final Long currencyId) throws GateException
	{
		try
		{
			return GateRSV51Executor.getInstance().execute(new HibernateAction<CurrencyImpl>()
			{
				public CurrencyImpl run(Session session) throws Exception
				{
					Query query = session.getNamedQuery("com.rssl.phizicgate.rsV51.money.CurrencyHelper.getCurrencyById");
	...
	public CurrencyImpl getCurrencyByCode(final String code) throws GateException
	{
		try
		{
			return GateRSV51Executor.getInstance().execute(new HibernateAction<CurrencyImpl>()
			{
				public CurrencyImpl run(Session session) throws Exception
				{
					Query query = session.getNamedQuery("com.rssl.phizicgate.rsV51.money.CurrencyHelper.getCurrencyByCode");
	...
	public List<CurrencyImpl> getCurrencies() throws GateException
	{
		try
		{
			return GateRSV51Executor.getInstance().execute(new HibernateAction<List<CurrencyImpl>>()
			{
				public List<CurrencyImpl> run(Session session) throws Exception
				{
					Query query = session.getNamedQuery("com.rssl.phizicgate.rsV51.money.CurrencyHelper.GetCurrencies");
					return query.list();
				}
			});
		}
