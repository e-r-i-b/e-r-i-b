...
(function($) {
    ...
    function jqPlot() {
            ...
            for (var n in this.axes) {
                ...
                if (axis.borderColor == null) {
                    if (n != 'xaxis' && n != 'x2axis' && axis.useSeriesColor === true && axis.show) {
                        ...
                        // если существует неготивный цвет оси, разукрашиваем ось в два цвета
                        if (axis._series[0].negativeSeriesColors && axis._series[0].negativeSeriesColors[0])
                            axis.borderNegativeColor = axis._series[0].negativeSeriesColors[0];
                    }
                    else {
        ...
        function getEventPosition(ev) {
            ...
            if (ev.offsetX) //чтобы правильно выскакивали сектора (пр: Safari)
            {
                gridPos = {x:ev.offsetX, y:ev.offsetY};
            }
   ...
   $.jqplot.ColorGenerator = function(colors) {
    ...
    $.jqplot.CanvasGridRenderer.prototype.draw = function() {
        ...
        if (this.borderWidth != 0 && this.drawBorder) {
            ...
            // разноцветная ось y
            var zeroPos = this._bottom
            if (axes.yaxis.borderNegativeColor)
            {
                zeroPos =Math.round(axes.yaxis.u2p(0)) + 0.5;
                // до нуля
                drawLine (this._left, this._bottom, this._left, zeroPos, {lineCap:'round', strokeStyle:axes.yaxis.borderNegativeColor, lineWidth:axes.yaxis.borderWidth});
            }
            // после нуля
            drawLine (this._left, zeroPos, this._left, this._top, {lineCap:'round', strokeStyle:axes.yaxis.borderColor, lineWidth:axes.yaxis.borderWidth});
        }
    ...
    $.jqplot.MarkerRenderer = function(options){
        ...
        this.show = true;
        // prop: style
        // One of diamond, circle, square, x, plus, dash, filledDiamond, filledCircle, filledSquare, filledBorderCircle
        this.style = 'filledCircle';
        // prop: fillColor
        // для filledBorderCircle задает цвет внутрений заливки
        this.fillColor = "#ffffff";
        // prop: lineWidth
        // size of the line for non-filled markers. Для filledBorderCircle используется в качестве обозначения ширины бордюра
        this.lineWidth = 2;
        // prop: size
    ...
    $.jqplot.MarkerRenderer.prototype.drawCircle = function(x, y, ctx, fill, options) {
        ...
        var end = 2*Math.PI;
        // даное малое число решает проблему с отображение кругов в ие, которые при
        // загадочных абстоятельствах иногда не отображаються
        end += 0.000005
        ...
        if (options.show == null || options.show != false) {
            ...
            switch (this.style) {
                ...
                case 'filledCircle':
                    ...
                    break;
                case 'filledBorderCircle':
                    this.drawCircle(x,y,ctx, true, options);
                    var fillOptions = options;
                    fillOptions.fillStyle = this.fillColor;
                    this.size = this.size-this.lineWidth;
                    this.drawCircle(x,y,ctx, true, fillOptions);
                    this.size = this.size+this.lineWidth;
                    break;
                case 'square':
