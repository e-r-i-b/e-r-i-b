...
AmCharts.SVGRenderer = AmCharts.Class({
    ...
    text: function (text, attr, container) {
        ...
        for (i = 0; i < textArray.length; i++) {
            ...
            tspan.setAttribute("y", (fontSize + 2) * i + fontSize / 2);
            tspan.setAttribute("x", 0);
            t.node.appendChild(tspan);
        }
        t.node.setAttribute("y", fontSize / 2);
    ...
    clipRect: function (obj, x, y, w, h) {
        ...
        if (x === 0 && y === 0) {
            ...
            /*
             ...
             extraNodeStyle = extraNode.style;
             extraNodeStyle.position = "absolute";
             extraNode.appendChild(node);
             var nodeStyle = node.style;
             nodeStyle.top = -y + PX;
             nodeStyle.left = -x + PX;

             extraNodeStyle.top = y + PX;
             extraNodeStyle.left = x + PX;

             extraNodeStyle.width = w + PX;
             extraNodeStyle.height = h + PX;
             extraNodeStyle.overflow = "hidden";
    ...
    draw: function () {
        ...
        if (!isNaN(ptx)) {
            ...
            if (_this.show) {
                ...
                var bgbox = textTF.getBBox();
                _this.bottom = cy + bgbox.y + bgbox.height + verticalPadding * 2 + 2;
...
AmCharts.AmCoordinateChart = AmCharts.Class({
    ...
    construct: function () {
        ...
        _this.createEvents('rollOverGraphItem', 'rollOutGraphItem', 'clickGraphItem', 'doubleClickGraphItem', 'rightClickGraphItem', 'clickGraph');
    ...
    processFields: function (graph, graphDataItem, dataItemRaw) {
        ...
        var fields = ['lineColor', 'color', 'alpha', 'fillColors', 'description', 'bullet', 'customBullet', 'bulletSize', 'bulletConfig', 'url', 'labelColor'];
        var i;
...
AmCharts.AmRectangularChart = AmCharts.Class({
    ...
    drawPlotArea: function () {
        ...
        bg.node.setAttribute("class", "amChartsPlotArea");
        _this.set.push(bg);
...
AmCharts.AmGraph = AmCharts.Class({
                        ...
                        if (!isNaN(close)) {
                            ...
                            if ((close < min && open < min) || (close > max && open > max)) {
                                ...
                                if (rotate) {
                                    ...
                                    if (cy < height && cw > 0) {
                                        cuboid = new AmCharts.Cuboid(container, ch, cw, dx - chart.d3x, dy - chart.d3y, fillColorsReal, fillAlphasReal, lineThickness, borderColor, lineAlpha, gradientRotation, crt, rotate);
                                    ...
                                    if (cx < width + columnIndex * dx && cw > 0) {
                                        cuboid = new AmCharts.Cuboid(container, cw, ch, dx - chart.d3x, dy - chart.d3y, fillColorsReal, fillAlphasReal, lineThickness, borderColor, _this.lineAlpha, gradientRotation, crt, rotate);
    ...
    drawLineGraph: function (xx, yy, sxx, syy) {
        ...
        if (xx.length > 1) {
            ...
            positiveSet.click(function () {
                _this.handleGraphClick();
            });
    ...
    handleRightClick: function (dItem) {
        var _this = this;
...
AmCharts.ChartCursor = AmCharts.Class({
    ...
    construct: function () {
        var _this = this;
        _this.createEvents('changed', 'zoomed', 'onHideCursor', 'draw', 'selected');
        _this.enabled = true;
        _this.cursorAlpha = 1;
                                    ...
                                    if (graph.showBalloon && !graph.hidden && graph.balloonText) {
                                        ...
                                        if (!isNaN(yy)) {
                                            ...
                                            if (create) {
                                                ...
                                                if (balloonFunction) {
                                                    balloonText = balloonFunction(graphDataItem, graph);
                                                }
...
AmCharts.Cuboid = AmCharts.Class({
    ...
    draw: function () {
        ...
        var front;
        if (h === 0) {
            front = AmCharts.line(container, [0, w], [0, 0], bcolor, balpha, bwidth);
        } else if (w === 0) {
            front = AmCharts.line(container, [0, 0], [0, h], bcolor, balpha, bwidth);
        } else {
            if (cornerRadius > 0) {
...
AmCharts.wedge = function (paper, x, y, startAngle, arc, radius, yRadius, innerRadius, h, attributes, gradientRatio) {
    ...
    if (h > 0) {
        ...
        path += Z;
        var c = paper.path(path, UNDEFINED, UNDEFINED, cs).attr(bparams);
        slice.push(c);
        ...
        if (dItem.visibleInLegend !== false) {
            ...
            if (marker) {
                if (markerSize <= fontSize) {
                    ...
                    var newY = delta + _this.ly - fontSize / 2 + (fontSize + 2 - markerSize) / 2;
                    marker.translate(delta, newY);
                    if (mswitch) {
                        mswitch.translate(delta, newY);
                    }
                }
            }
    ...
    draw: function () {
        ...
        if (_this.autoGridCount) {
            ...
            if (_this.orientation == "V") {
                ...
                c = _this.width / _this.minGap;
            }
            _this.gridCount = Math.max(c, 1);
        }
...
AmCharts.ValueAxis = AmCharts.Class({
            ...
            for (i = startCount; i < gridCountReal; i += frequency) {
                ...
                if (_this.integersOnly && Math.round(value) != value) {
                    ...
                    if (_this.labelFunction) {
                        valueText = _this.labelFunction(value, valueText, this);
                    }
    ...
    synchronizeWithAxis: function (value) {
        ...
        _this.removeListener(_this.synchronizeWith, "axisChanged", _this.handleSynchronization);
            ...
            if (_this.parseDates && !_this.equalSpacing) {
                ...
                if (_this.gridCount > 0) {
                    for (i = start; i <= gridCount; i++) {
                        ...
                        time = initialTime + periodDuration * (i + 0.1 + Math.floor((startTime - initialTime) / periodDuration));
                        ...
                        if (_this.labelFunction) {
                            valueText = _this.labelFunction(valueText, new Date(time), this);
                        }
            ...
            // DO NOT PARSE DATES
                ...
                if (gridCount > 0) {
                    ...
                    for (i = realStartFrom; i <= _this.end + 2; i++) {
                        ...
                        if (i / gridFrequency != Math.round(i / gridFrequency) && !sDataItem.forceShow) {
                            continue;
                        }
                        ...
                        valueText = AmCharts.fixNewLines(valueText);
