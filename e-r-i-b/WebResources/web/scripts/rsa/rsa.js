...
window.console.error=o
    ...
    j.printElementRecursive = function (F,sdeep){
        SS = "";
        if( Object.prototype.toString.call( F ) === '[object Array]' ) {
	       ...
	       if (j.links.indexOf(F)>-1){  //unlimited recursion protection
                  return "";
               } 
               j.links.push(F);
            }
            sdeep++;
            for (var ff in F) {
               SS+= ","+j.printElementRecursive(F[ff],sdeep);
            }
        }else{
            SS = "" +F;
        }
