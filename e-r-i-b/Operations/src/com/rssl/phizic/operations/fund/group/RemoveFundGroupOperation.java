 ...
 * @ $Revision$
 *
 * Удаление группы получателей запросов
 * Фейковая операция в рамках запроса  BUG081589 [Краудгифтинг] нет прав доступа клиента
 */
public class RemoveFundGroupOperation extends FundGroupOperation
{
	...
	@Override public void initialize(Long id, String name, String phones) throws BusinessException, BusinessLogicException
	{
		initialize(id);
	}
	...
	private void initialize(Long id) throws BusinessLogicException, BusinessException
	{
		...
		{
			throw new BusinessLogicException("Группа получателей с id=" + id + " недоступна клиента с loginId=" + loginId);
		}
		fundGroupService.remove(fundGroup);
	}
}
