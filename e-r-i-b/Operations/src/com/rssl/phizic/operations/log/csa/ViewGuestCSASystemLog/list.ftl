...
FROM (
    ...
    WHERE
        ...
        <#if fio?has_content>
        <#--Опорный элемент: I_G_CSA_SL_FIO
            Предикаты доступа: access(UPPER(REPLACE(REPLACE("SUR_NAME"||"FIRST_NAME"||"PATR_NAME",' ',''),'-','')) LIKE
            ...
            Кардинальность: количество записей зарегистрированного гостя за период(стоп условие - пагинация: 10, 20, 50)
        -->
        AND (
        ...
        <#--Опорный элемент: I_G_CSA_SL_PHONE_DATE
            Предикаты доступа: access("GUESTLOG"."PHONE_NUMBER"=:EXTRA_PHONENUMBER AND
                           SYS_OP_DESCEND("START_DATE")>=SYS_OP_DESCEND(TO_TIMESTAMP(:EXTRA_TODATE)) AND
                           SYS_OP_DESCEND("START_DATE")<=SYS_OP_DESCEND(TO_TIMESTAMP(:EXTRA_FROMDATE)))
            Кардинальность: количество записей гостя за период(стоп условие - пагинация: 10, 20, 50)
        -->
