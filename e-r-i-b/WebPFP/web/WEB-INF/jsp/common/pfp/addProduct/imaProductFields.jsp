...
<tiles:insert definition="formRow" flush="false">
    ...
    <tiles:put name="data">
        <c:choose>
            ...
            <c:otherwise>
                <span class="bold">
                    <c:set var="IMAIncome"><bean:write name="product" property="defaultIncome"/></c:set>
                    <html:hidden property="field(imaIncome)" value="${IMAIncome}"/>
                    ${IMAIncome}<bean:message bundle="pfpBundle" key="label.addProduct.income.help"/>
...
<script type="text/javascript">
    ...
    function calculateIncomeAmount()
    {
        ...
        var amount = parseFloatVal($('input[name="field(imaAmount)"]').val());
        if(isNaN(amount))
            amount = 0;
        var income = parseFloatVal($('input[name="field(imaIncome)"]').val());
