...
import com.rssl.phizic.gate.notification.NotificationConfig;
...
public class NotificationServiceImpl extends AbstractService implements NotificationService
{
	...
	public void deleteGarbageNotification() throws GateException
	{
		try
		{
			RSRetailV6r20Executor.getInstance(true).execute(new HibernateAction<Void>()
			{
				public Void run(Session session) throws Exception
				{
					NotificationConfig config = ConfigFactory.getConfig(NotificationConfig.class);
					Query query = session.getNamedQuery("com.rssl.phizicgate.rsretailV6r20.notification.GetGarbageNotifications");
					Date date = DateHelper.add(DateHelper.getCurrentDate().getTime(), 0, 0, -config.getDaysToKeepNotification());
					Calendar calendar = DateHelper.toCalendar(date);
					query.setParameter("cutDate", calendar);
	...
	private List<Notification> notificationQueryDispatcher(final Class<? extends Notification> notificationType, final Map<String, Object> params) throws GateException
	{
		try
		{
			return RSRetailV6r20Executor.getInstance(true).execute(new HibernateAction<List<Notification>>(){
