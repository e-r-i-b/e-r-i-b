...
import com.rssl.phizicgate.rsretailV6r20.data.BlobReadHibernateAction;
...
public class BlobTest extends RSRetailV6r20GateTestCaseBase
{
	public void testGetBlobValue() throws Exception
	{
		...
		try
		{
			resultTest = RSRetailV6r20Executor.getInstance().execute(new BlobReadHibernateAction<TestV6>("drt_paym_1",
					new HibernateAction<TestV6>()
					{
						public TestV6 run(Session session) throws Exception
						{
							Query namedQuery = session.getNamedQuery("com.rssl.phizicgate.rsretailV6r20.test.getBlobTest");
		...
		try
		{
			resultList = RSRetailV6r20Executor.getInstance().execute(new BlobReadHibernateAction<List<TestV6>>("drt_paym_1",
					new HibernateAction<List<TestV6>>()
					{
						public List<TestV6> run(Session session) throws Exception
						{
							Query namedQuery = session.getNamedQuery("com.rssl.phizicgate.rsretailV6r20.test.getBlobTest.list");
