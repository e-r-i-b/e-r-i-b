package com.rssl.phizicgate.rsretailV6r4.job;
...
import com.rssl.phizicgate.rsretailV6r4.senders.ExternalKeyCreator;
	...
	public void execute(JobExecutionContext context)
	{
		...
		try
		{
			...
			for (int i = 0; i < len; ++i)
			{
				...
				//Из макроса приходят все непроведенные платежи, если synchronizableDocument == null,
				//значит платеж совершен клиентом с другой базы
				if (synchronizableDocument == null)
				{
					log.info("Документ с applicationKey " + applicationKey + " не найден");
					continue;
				}
				...
				if(OK_ERROR_CODE.equals( errorCode))
				{
					...
					log.debug("Обновлякм в бизнесе проведенный в гейте платеж "+synchronizableDocument.getId());
