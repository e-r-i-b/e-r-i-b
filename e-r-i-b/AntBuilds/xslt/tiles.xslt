...
<xsl:variable name="template" select="document($templateFile)"/>
...
# Global   : tiles/tiles-global-templates.xml, config/global-tiles-definitions.xml
...
<xsl:template match="/tiles-definitions">
	<xsl:copy>
		<xsl:comment>##### from tiles/tiles-global-templates.xml ####</xsl:comment>
		<xsl:apply-templates select="$template//definition"/>
...
<xsl:template name="validateOverride">
	...
	<xsl:variable name="templateElem" select="exsl:key($key, $value, $template)"/>

	<xsl:if test="not(boolean($globalElem)) and not(boolean($templateElem))">
		<xsl:message terminate="yes">
##############################################################################################################
# В global-tiles-definitions.xml и tiles-global-templates.xml не определен 
...
</xsl:stylesheet><!-- Stylesheet edited using Stylus Studio - (c) 2004-2005. Progress Software Corporation. All rights reserved. --><!-- Stylesheet edited using Stylus Studio - (c) 2004-2005. Progress Software Corporation. All rights reserved. --><!-- Stylesheet edited using Stylus Studio - (c) 2004-2005. Progress Software Corporation. All rights reserved. -->