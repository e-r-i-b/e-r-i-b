...
<c:if test="${not empty person && name!=''}">
    <${name}>
        ...
        <c:if test="${phiz:getApiVersionNumber().major >= 9}">
            <loginId><bean:write name="person" property="login.id" ignore="true"/></loginId>

            <c:set var="tarif" value="${person.tarifPlanCodeType}"/>
            <TARIF_PLAN>${empty tarif ? 0 : tarif}</TARIF_PLAN>
        </c:if>
    </${name}>
</c:if>
