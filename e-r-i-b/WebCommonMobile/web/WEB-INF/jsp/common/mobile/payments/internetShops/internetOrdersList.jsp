    ...
    <tiles:insert definition="iphone" flush="false">
        <tiles:put name="data">
            <sl:collection id="internetOrder" property="data" model="xml-list" title="orderList">
                <c:set var="internetOrderId" value="${internetOrder.id}"/>
                ...
                <c:set var="paymentId" value=""/>
                <c:set var="stateCode" value="${internetOrder.state}"/>

                <c:choose>
                    <c:when test="${internetOrder.kind == 'AEROFLOT'}">
                        <c:set var="formName" value="AirlineReservationPayment"/>
                    </c:when>
                    <c:when test="${internetOrder.kind == 'INTERNET_SHOP'}">
                        <c:set var="formName" value="ExternalProviderPayment"/>
                    </c:when>
                    <c:otherwise>
                        <c:set var="formName" value="FNSPayment"/>
                    </c:otherwise>
                </c:choose>
                ...
                <sl:collectionItem title="order">
                    ...
                    <orderState>
                        <c:choose>
                            <c:when test="${stateCode == 'CREATED' or stateCode == 'RELATED' or stateCode == 'PAYMENT'}">SAVED</c:when>
                            <c:when test="${stateCode == 'REFUSED' or stateCode == 'REFUND' or stateCode == 'PARTIAL_REFUND' or stateCode == 'DELAYED'}">CANCELED</c:when>
                            <c:when test="${stateCode == 'WRITE_OFF' or stateCode == 'EXECUTED' or stateCode == 'ERROR'}">DISPATCHED</c:when>
