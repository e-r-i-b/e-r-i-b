...
public class EditMailMobileAction extends EditMailAction
{
	...
	protected FieldValuesSource getFormProcessorValueSource(EditFormBase frm, EditEntityOperation operation) throws Exception
	{
        ...
        setPhoneField(fields, form, operation);  // обновить телефон данными из черновика
    ...
    private void setPhoneField(Map<String, Object> fields, EditMailMobileForm form, EditEntityOperation operation) throws BusinessLogicException, BusinessException
    {
        if(form.getId() != null && operation.getEntity() != null && "BY_PHONE".equals(form.getResponseMethod()) && isFormPhoneMasked(form.getPhone())) {
            fields.put("phone",((Mail) operation.getEntity()).getPhone());
        }
        else {
    ...
    private boolean isFormPhoneMasked(String phone)
    {
        return phone == null ? false : phone.contains("•••");
    }
