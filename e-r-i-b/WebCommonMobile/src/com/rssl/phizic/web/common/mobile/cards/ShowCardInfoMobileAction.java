...
public class ShowCardInfoMobileAction extends ShowCardInfoAction
{
    ...
    protected void updateFormData(ViewEntityOperation operation, EditFormBase form) throws BusinessException, BusinessLogicException
	{
        ...
        if (checkAccess(MoneyBoxListOperation.class, "MoneyBoxManagement"))
        {
	        ...
	        //исключаем черновики: includeTemplates = false.
            moneyBoxListOperation.initialize(cardLink, false);
            frm.setMoneyBoxesTargets(moneyBoxListOperation.getMoneyBoxesTargets());
        }

        if (checkAccess(ListAutoSubscriptionLinksOperation.class, "AutoSubscriptionLinkManagment") && checkAccess(ListAutoSubscriptionLinksOperation.class, "ClientAutotransfersManagement")) {
            ...
            listAutoSubscriptionLinksOperation.initialize();
            frm.setP2pSubscriptions(listAutoSubscriptionLinksOperation.getP2pSubscriptionsLinks(cardLink));
        }
