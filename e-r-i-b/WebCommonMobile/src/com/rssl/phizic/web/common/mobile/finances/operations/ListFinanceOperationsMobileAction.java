...
public class ListFinanceOperationsMobileAction extends GetListOfOperationsAction
{
	...
	protected void doFilter(Map<String, Object> filterParams, ListEntitiesOperation operation, ListFormBase frm) throws Exception
	{
		...
		List<CardOperationDescription> operationDescriptions = op.getCardOperationsToCategories(normalizedFilterParams, maxResults, form.getPaginationOffset(), false);

		if (!(Boolean) filterParams.get("showOtherAccounts"))
		{
			CollectionUtils.filter(operationDescriptions, new Predicate()
			{
				public boolean evaluate(Object object)
				{
					CardOperationDescription description = (CardOperationDescription) object;

					if (!description.getCard().isMain() && !description.getCategory().isForInternalOperations())
					{
						return false;
					}
		...
		form.setCardOperations(operationDescriptions);
	}
}
