...
<c:set  scope="request" var="serviceName"               value="DelayEmployeeAutoSubscriptionPayment"/>

<c:set  scope="request" var="subMenu"                   value="AutoSubscriptions"/>

<c:set  scope="request" var="title"                     value="Заявка на приостановление выполнения автоплатежа."/>
<c:set  scope="request" var="description"               value="Для того чтобы приостановить автоплатеж, проверьте реквизиты заявки и нажмите на кнопку «Сохранить». В результате после исполнения операции автоплатеж будет приостановлен."/>
...
<c:set scope="request" var="printActionType">
    <bean:message key="actionType.stop" bundle="autopaymentsBundle"/>
</c:set>
<c:set scope="request" var="printActionDescription">
    <bean:message key="actionType.stop.description" bundle="autopaymentsBundle"/>
</c:set>
...
<c:set  scope="request" var="isDelayClaim"              value="${true}"/>
