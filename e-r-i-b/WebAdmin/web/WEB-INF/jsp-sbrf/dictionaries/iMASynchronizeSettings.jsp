...
<html:form action="/dictionaries/synchronize/ima_settings" onsubmit="return setEmptyAction(event);">
    ...
    <tiles:insert definition="propertiesForm">
        ...
        <tiles:put name="submenu" type="string" value="IMASettings"/>
        <tiles:put name="pageName" type="string" value="Настройка ОМС для загрузки"/>
        <tiles:put name="pageDescription"
                   value="Используйте данную форму для настройки загрузки ОМС."/>
        <tiles:put name="formAlign" value="center"/>
        <tiles:put name="data">

            <table>
                <tiles:insert definition="propertyField" flush="false">
                    <tiles:put name="fieldName" value="com.rssl.iccs.imaproduct.uploading.mode"/>
                    <tiles:put name="showHint" value="none"/>
                    <tiles:put name="fieldType" value="radio"/>
                    <tiles:put name="selectItems" value="ALL@Все виды|OPTIONALLY@Укажите номера видов ОМС для загрузки:"/>
            ...
            <c:choose>
                <c:when test="${showCheckbox == 'true'}">
                    <div class="propertySplitter">
                        <input type="checkbox" id="replicationSelect" value="com.rssl.iccs.imaproduct.kinds.allowed.uploading" name="selectedProperties" style="display: none;"/>
                ...
                <c:otherwise>
                    ...
                    <div id="titleRow">
                        <p><b>Загружать следующие номера ОМС:</b></p>
                    </div>
                    ...
                    <div id="numbersRow">
                        <div id="cardSynchronizeSettingsDiv" class="align-center" style="display: none;">
        ...
        <tiles:put name="formButtons">
            <tiles:insert definition="commandButton" flush="false"
                          operation="EditIMASynchronizeSettingsOperation">
    ...
    <script type="text/javascript">
        var wrongNumberMessage = "Указанный Вами код не соответствует кодам ОМС. Пожалуйста, введите другой код."
        doOnLoad(function() {
            init();
        ...
        {
            var radioValue = getRadioValue(document.getElementsByName('field(com.rssl.iccs.imaproduct.uploading.mode)'));
        ...
        function removeNumbers()
        {
            ...
            if(!checkSelection("chkName", "Для удаления вида ОМС выберите одну или несколько записей."))
                return;
         ...
         * -1 если вид не относится к ОМС (com.rssl.iccs.ima.kinds.allowed.downloading)
        ...
        function addOrUpdateNumber(editingNumber, update)
        {
            ...
            <c:forEach var="entry" items="${form.imaTypes}">
                if(kind == ${entry})
                    notIMA = false;
            </c:forEach>
            if (notIMA)
        ...
        {
            return "Добавить вид (подвид) ОМС для загрузки";
        }
        function getHelpInfo()
        {
            return "Для добавления вида ОМС укажите код вида и код подвидов ОМС и нажмите на кнопку «Сохранить».";
        }
        function getLabel1()
        {
            return "Код вида ОМС*";
        ...
        function getLabel2()
        {
            return "Код подвида ОМС";
        }
        function isIMAforLoading()
        {
            return true;
        }
    </script>
