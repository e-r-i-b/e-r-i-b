...
<html:form action="/persons/receivers/list" onsubmit="return setEmptyAction(event);">
	...
	<tiles:insert definition="personEdit">
		...
		<tiles:put name="pageTitle" type="string" value="Список получателей юр.лиц."/>
		...
		<tiles:put name="menu" type="string">
			...
			<c:if test="${isShowSaves}">
				<tiles:insert definition="clientButton" flush="false" operation="EditPaymentReceiverOperationAdmin">
					...
					<tiles:put name="action"  value="/persons/receivers/edit.do?person=${param.person}&kind=J"/>
                    ...
                    <sl:collection id="listElement" model="list" property="data" bundle="personsBundle">
                        ...
                        <sl:collectionItem title="label.receiver.fio"   value="${listElement.name}">
                            <sl:collectionItemParam
                                    id="action"
                                    value="/persons/receivers/edit.do?person=${param.person}&kind=G&id=${listElement.id}"
                                    condition="${phiz:impliesOperation('EditPaymentReceiverOperationAdmin','PersonManagement')}"
                                    />
                        </sl:collectionItem>
                        <sl:collectionItem title="label.receiver.inn">
                            &nbsp;${listElement.INN}
                            <c:if test="${not empty listElement.KPP}">
                                &nbsp;/&nbsp;${listElement.KPP}
                            </c:if>
                        </sl:collectionItem>
                        ...
                        <sl:collectionItem title="label.receiver.commission" value="${listElement.commision}"/>
                    </sl:collection>
                </tiles:put>
