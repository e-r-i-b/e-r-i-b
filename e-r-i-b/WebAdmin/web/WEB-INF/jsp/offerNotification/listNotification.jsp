...
<html:form action="/offers/notification/list" onsubmit="return setEmptyAction(event);">
    <c:set var="form" value="${phiz:currentForm(pageContext)}"/>
    <tiles:insert definition="OfferNotificationList">
        <tiles:put name="pageTitle" type="string">
            <bean:message key="list.title" bundle="offerNotificationBundle"/>
        </tiles:put>
        <tiles:put name="menu" type="string">
            <tiles:insert definition="clientButton" flush="false" operation="EditOfferNotificationOperation">
                ...
                <tiles:put name="action"  value="/offers/notification/edit.do"/>
            ...
            <tiles:insert definition="filterEntryField" flush="false">
                ...
                <tiles:put name="data">
                    <html:select property="filter(state)" styleClass="select">
                        <html:option value=""><bean:message key="label.filter.all" bundle="offerNotificationBundle"/></html:option>
                        <html:option value="ACTIVE"><bean:message key="label.filter.active" bundle="offerNotificationBundle"/></html:option>
                        <html:option value="NOTACTIVE"><bean:message key="label.filter.notactive" bundle="offerNotificationBundle"/></html:option>
                    </html:select>
                </tiles:put>
            </tiles:insert>
        </tiles:put>
        ...
        <tiles:put name="data" type="string">
            ...
            <tiles:insert definition="tableTemplate" flush="false">
                <tiles:put name="id" value="OfferNotificationsList"/>

                <%-- Данные таблицы --%>
                <tiles:put name="grid">
                    <sl:collection id="listElement" model="list" property="data" bundle="offerNotificationBundle">
                        <sl:collectionParam id="selectType" value="checkbox"/>
                        <sl:collectionParam id="selectName" value="selectedIds"/>
                        <sl:collectionParam id="selectProperty" value="id"/>

                        <sl:collectionItem title="label.list.period" styleClass="Width160">
                        ...
                        <sl:collectionItem title="label.list.name">
                            <c:if test="${not empty listElement and not empty listElement.name}">
                                <phiz:link action="/offers/notification/edit"
                                           operationClass="EditOfferNotificationOperation">
                                    <phiz:param name="id" value="${listElement.id}"/>
                                    <bean:write name="listElement" property="name"/>
                                </phiz:link>
                            </c:if>
                        </sl:collectionItem>
                        <sl:collectionItem title="label.list.tb" styleClass="Width160">
                            <c:if test="${not empty listElement and not empty listElement.departments}">
                                ${phiz:departmentsToString(listElement.departments, 25)}
                            </c:if>
                        </sl:collectionItem>
                        <sl:collectionItem title="label.list.orderIndex" styleClass="Width120">
                            <c:if test="${not empty listElement and not empty listElement.orderIndex}">
                                <bean:write name="listElement" property="orderIndex"/>
                            </c:if>
                        </sl:collectionItem>
                        <sl:collectionItem title="label.list.state" styleClass="Width120">
                            <c:if test="${not empty listElement and not empty listElement.state}">
                                <c:if test="${listElement.state == 'ACTIVE'}"><bean:message key="label.active" bundle="offerNotificationBundle"/></c:if>
                                <c:if test="${listElement.state == 'NOTACTIVE'}"><bean:message key="label.notactive" bundle="offerNotificationBundle"/></c:if>
                            </c:if>
                        </sl:collectionItem>
                    </sl:collection>
                </tiles:put>

                <tiles:put name="emptyMessage"><bean:message key="emptyMessage" bundle="offerNotificationBundle"/></tiles:put>
                <script type="text/javascript">
                    var addUrl = "${phiz:calculateActionURL(pageContext,'/offers/notification/edit')}";
                ...
                <tiles:put name="buttons">
                    ...
                    <tiles:insert definition="commandButton" flush="false" operation="RemoveOfferNotificationOperation">
                        ...
                        <tiles:put name="validationFunction">
                            ...
                            {
                            checkIfOneItem("selectedIds");
                            return checkSelection('selectedIds', 'Выберите уведомления для удаления!');
                            }
                        </tiles:put>
                        <tiles:put name="confirmText"><bean:message key="confirm.text" bundle="offerNotificationBundle"/></tiles:put>
                    </tiles:insert>
                </tiles:put>
