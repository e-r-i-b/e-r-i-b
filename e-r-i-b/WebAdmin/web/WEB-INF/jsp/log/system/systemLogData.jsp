        ...
        <sl:collection id="listElement" model="wide-list" property="list" bundle="logBundle" styleClass="standartTable">
            ...
            <sl:collectionItem title="label.id">
                <c:if test="${not empty logEntry}">
                    <c:choose>
                        <c:when test="${empty logEntry.loginId or logEntry.loginId == -1}">
                            &nbsp;
                        </c:when>
                        ...
                        <c:otherwise>
                            <c:if test="${logEntry.userId == 'admin'}">0</c:if>
                            <c:if test="${logEntry.userId != 'admin'}">
                                ${logEntry.userId}
                            </c:if>
                        </c:otherwise>
                    </c:choose>
            ...
            <sl:collectionItem title="label.session.id">
                <c:choose>
                    <c:when test="${empty logEntry.sessionId or  logEntry.sessionId == 'none'}">
                        &nbsp;
                    </c:when>
                    <c:otherwise>
                         <bean:write name="logEntry" property="sessionId"/>&nbsp;
                    </c:otherwise>
                </c:choose>
            </sl:collectionItem>
