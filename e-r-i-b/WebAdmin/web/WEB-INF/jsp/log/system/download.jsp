...
<html>
    ...
    <h1>
        <span style="font-family:MS Sans Serif; font-size:10pt;font-weight:bold">
            ...
            <bean:write name="DownloadSystemLogForm" property="filter(fromDate)" format="dd.MM.yyyy"/> <bean:write name="DownloadSystemLogForm" property="filter(fromTime)" format="HH:mm:ss"/>
            по <bean:write name="DownloadSystemLogForm" property="filter(toDate)" format="dd.MM.yyyy"/> <bean:write name="DownloadSystemLogForm" property="filter(toTime)" format="HH:mm:ss"/>
	...
	<table border="1" cellspacing="0" cellpadding="1" style="font-family:MS Sans Serif; font-size:8pt;" id="tableTitle">
			...
			<!-- строки списка -->
			<c:forEach items="${DownloadSystemLogForm.list}" var="listElement">
				...
				<tr>
					...
					<td nowrap="true">&nbsp;
						<c:choose>
							...
							<c:when test="${logEntry.application=='Messaging'}">
								<bean:message key="system.log.application.messaging" bundle="logBundle"/>
							</c:when>
							<c:when test="${logEntry.application=='none'}">
