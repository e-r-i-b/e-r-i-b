...
public class DownloadLogFilterForm extends DownloadLogFilterBaseForm
{
	...
	public static Form OPERATION_LOG_FILTER_FORM = createFilterForm();
	...
	protected static final FormBuilder getPartlyFormBuilder()
	{
		...
		fieldBuilder.setValidators(new DateFieldValidator(TIMESTAMP_FORMAT, "Время должно быть в формате ЧЧ:ММ:СС"));
		fieldBuilder.setParser(timeParser);
		formBuilder.addField(fieldBuilder.build());
	...
	private static Form createFilterForm()
	{
		...
		fieldBuilder.addValidators(new RegexpFieldValidator("^\\d*$","Login_ID должен содержать только цифры"));
		formBuilder.addField(fieldBuilder.build());
