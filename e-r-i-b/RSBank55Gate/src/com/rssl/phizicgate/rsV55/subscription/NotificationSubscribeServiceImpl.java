...
import com.rssl.phizicgate.rsV55.money.CurrencyHelper;
...
public class NotificationSubscribeServiceImpl extends AbstractService implements NotificationSubscribeService
{
		...
		try
        {
            GateRSV55Executor.getInstance().execute(new HibernateAction<Void>()
            {
                public Void run(Session session) throws Exception
                {
	                ...
	                Query query = session.getNamedQuery("com.rssl.phizicgate.rsV55.subscription.FindMaxSubscriptionNumber");
	...
	private SubscriptionObject findAccountSubscription(final Account account) throws GateException
	{
       try
        {
            return GateRSV55Executor.getInstance(true).execute(new HibernateAction<SubscriptionObject>()
            {
                public SubscriptionObject run(Session session) throws Exception
                {
	                Query query = session.getNamedQuery("com.rssl.phizicgate.rsV55.subscription.FindAccountSubscription");
