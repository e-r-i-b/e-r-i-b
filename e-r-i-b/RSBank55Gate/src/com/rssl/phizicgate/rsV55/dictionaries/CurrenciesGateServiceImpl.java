...
import com.rssl.phizicgate.rsV55.money.CurrencyImpl;
...
public class CurrenciesGateServiceImpl extends AbstractService implements CurrenciesGateService
{
	...
	public List<Currency> getAll(final int firstResult, final int maxResults) throws GateException, GateLogicException
	{
		try
		{
			...
			List<CurrencyImpl> list = GateRSV55Executor.getInstance().execute(new HibernateAction<List<CurrencyImpl>>()
			   {
					public List<CurrencyImpl> run(Session session) throws Exception
					{
						 //noinspection unchecked
						 Query query = session
								   .getNamedQuery("com.rssl.phizicgate.rsV55.money.CurrencyHelper.GetCurrencies");
			...
			List<CurrencyImpl> list = GateRSV55Executor.getInstance().execute(new HibernateAction<List<CurrencyImpl>>()
			   {
					public List<CurrencyImpl> run(Session session) throws Exception
					{
						 //noinspection unchecked
						 Query query = session
								   .getNamedQuery("com.rssl.phizicgate.rsV55.money.CurrencyHelper.GetCurrenciesByTemplate");
