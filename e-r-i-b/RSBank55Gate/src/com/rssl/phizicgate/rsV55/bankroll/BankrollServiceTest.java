...
public class BankrollServiceTest extends RSRetaileGateTestCaselBase
{
	...
	public void testGetAdditionalCards() throws GateException
    {
        try{
	        BankrollServiceImpl service  = new BankrollServiceImpl(gateFactory);
			List<Card> cards= GroupResultHelper.getOneResult(service.getAdditionalCards(service.getCard(EXTERNAL_CARD_ID).getResult(EXTERNAL_CARD_ID)));
			assertNotNull(cards);
			for (Card card : cards)
			{
    ...
    public void testGetZStringsFromPervasive() throws GateException
    {
        try
        {
            List<String> zStrings = GateRSV55Executor.getInstance().execute(new HibernateAction<List<String>>()
            {
