...
public class BankrollServiceImpl extends AbstractService implements BankrollService
{
            ...
            throws GateException
    {
		try
        {
			return GateRSV55Executor.getInstance().execute(new HibernateAction<List<Account>>()
            {
    ...
    public AccountImpl getClientAccountByNumber(final String accountNumber, final String clientId) throws GateException {
        try
        {
            return GateRSV55Executor.getInstance().execute(new HibernateAction<AccountImpl>()
            {
    ...
    public GroupResult<Account, AccountInfo> getAccountInfo(Account... accounts)
    {
	    ...
	    for (final Account account: accounts)
	    {
			...
			try
			{
				res.putResult(account, GateRSV55Executor.getInstance().execute(new HibernateAction<AccountInfo>()
				{
	...
	private AccountAbstract getAccountAbstract(final Account account, final Long number) throws GateException
    {
        try
        {
            return GateRSV55Executor.getInstance().execute(new HibernateAction<AccountAbstract>()
            {
    ...
    public List<Card> getClientCards(final Client client) throws GateException
    {
        try
        {
            List<Card> cards = GateRSV55Executor.getInstance().execute(new HibernateAction<List<Card>>()
            {
    ...
    public GroupResult<String, Card> getCard(String... externalIds)
    {
	    ...
	    for (String externalId: externalIds)
	    {
			...
			try
			{
				CardImpl card = ( CardImpl ) GateRSV55Executor.getInstance().execute(new HibernateAction<Card>()
				{
    ...
    Money getAccountBalance(final Account account, final Calendar date) throws GateException
    {
        try
        {
            return GateRSV55Executor.getInstance().execute(new HibernateAction<Money>()
            {
    ...
    private List<AccountTransaction> getAccountTransactions_OLD(final Account account, final Calendar fromDate, final Calendar toDate) throws GateException
    {
        try
         {
             return GateRSV55Executor.getInstance().execute(new HibernateAction<List<AccountTransaction>>()
             {
    ...
    Currency getCardCurrency(final Long cardId) throws GateException
    {
        try
        {
            return GateRSV55Executor.getInstance().execute(new HibernateAction<Currency>()
            {
	    ...
	    for (Card card:cards)
        {
	        try
			{
				...
				final Long     primaryAccountId  = Long.valueOf(primaryAccount.getId());
					...
					res.putResult(card, GateRSV55Executor.getInstance().execute(new HibernateAction<CardInfoImpl>()
					{
    ...
    private CardAbstract getCardAbstract(final Card card, final Long number) throws GateException
    {
        try
        {
            return GateRSV55Executor.getInstance().execute(new HibernateAction<CardAbstract>()
            {
                public CardAbstract run(Session session) throws Exception
                {
	                ...
	                if (CollectionUtils.isEmpty(cardOperations))
	                    return cardAbstract;
    ...
    private List<CardOperation> getCardUnsettledOperations(final Card card, final Calendar fromDate, final Calendar toDate) throws GateException, GateLogicException
    {
        ...
        try
        {
            return GateRSV55Executor.getInstance().execute(new HibernateAction<List<CardOperation>>()
            {
	...
	public GroupResult<Card, Account> getCardPrimaryAccount(Card... cards)
    {
	    ...
	    for (Card card: cards)
        {
			...
			try
			{
				res.putResult(card, GateRSV55Executor.getInstance().execute(new HibernateAction<Account>()
				{
	...
	public GroupResult<Card, Client> getOwnerInfo(Card... cards)
	{
		...
		for (Card card: cards)
        {
			...
			try
			{
				Long clientId = GateRSV55Executor.getInstance().execute(new HibernateAction<Long>()
				{
	...
	private CardType getCardType(final Card card) throws GateException, GateLogicException
	{
		...
		try
		{
			...
			date = GateRSV55Executor.getInstance().execute(new HibernateAction<Calendar>()
			{
	...
	private Calendar getPreviousOperationDate(final Account account, final Calendar from) throws GateException
	{
        try
        {
            return GateRSV55Executor.getInstance().execute(new HibernateAction<Calendar>()
            {
                public Calendar run(Session session) throws Exception
                {
                    final Long id = Long.decode(account.getId());
	...
	/**
	 * Заполнение данных по СКС карты 
	 * @param card - карта
	 */
	private void fillPrimaryAccount(final CardImpl card) throws GateException, GateLogicException
	{
		try
		{
			Long primaryAccountExternalId = GateRSV55Executor.getInstance().execute(new HibernateAction<Long>()
			{
				public Long run(Session session) throws Exception
				{
