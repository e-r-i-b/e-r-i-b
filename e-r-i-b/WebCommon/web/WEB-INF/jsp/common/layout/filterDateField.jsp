<%--
  Created by IntelliJ IDEA.
  User: osminin
  Date: 22.08.2008
  Time: 9:49:26
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=windows-1251" language="java" %>
<%@ page contentType="text/html;charset=windows-1251" language="java" %>
<%@ taglib uri="http://jakarta.apache.org/struts/tags-html" prefix="html" %>
<%@ taglib uri="http://jakarta.apache.org/struts/tags-bean" prefix="bean" %>
<%@ taglib uri="http://jakarta.apache.org/struts/tags-logic" prefix="logic" %>
<%@ taglib uri="http://jakarta.apache.org/struts/tags-tiles" prefix="tiles" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri="http://rssl.com/tags" prefix="phiz" %>

<c:set var="countTd" value="${countTd+1}" scope="request"/>
<tiles:importAttribute/>
<td class="filter" valign="middle">
    <script type="text/javascript">
    addClearMasks(null, function(){
        clearInputTemplate("filter(${name})",DATE_MASK);
    });
    </script>
    <span class="${isFastSearch == 'true' ? 'fastSearch' : ''}">
    <c:if test="${not empty label}">
        <c:if test="${noWrapLabel}"><span style="white-space:nowrap;"></c:if>
		<bean:message key="${label}" bundle="${bundle}"/>
		<c:if test="${mandatory==true}">
			<span class="filterAsterisk" style="cursor:default;">*</span>
		</c:if>
        <c:if test="${noWrapLabel}"></span></c:if>
    </c:if>
    </span>
</td>
<td>
	<span class="${isFastSearch == 'true' ? 'fastSearch' : ''}" style="font-weight:normal;cursor:default;">
        <c:if test="${not empty label}">
             <input type="text" class="${className}${(needClearing == 'true' ? '' : ' notCleaningField')}" ${!editable ? 'readonly="readonly"' : ''}
                        size="${size}" name="filter(${name})"  maxlength="${maxlength}"
                        value="<bean:write name="org.apache.struts.taglib.html.BEAN" property="filters.${name}" format="dd.MM.yyyy"/>" />
        </c:if>
	</span>
</td>
<c:if test="${empty colCount || colCount == ''}"><c:set var="colCount" value="3"/></c:if>
<c:if test="${countTd>=colCount}">
	</tr>
	<tr >
	<c:set var="countTd" value="0" scope="request"/>
</c:if>