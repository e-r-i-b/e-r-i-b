...
<%--
    ...
    Если имя функция selectFunctionName не указано, то вместо нее используется onClickFunctionName.
        ...
        saveRegion - доступна ли возможность сохранения региона
--%>
...
<tiles:importAttribute/>
    ...
    <c:if test="${count > 0 }">
        ...
        <c:if test="${not empty navigations }">
            <div class="clear"></div>
            <a class="alphabetOnClickLink" href="#" onclick="${onClickFunctionName}(0, '', '', ${saveRegion}); return false;"><b>${breadTitleAll}</b></a>
                    ...
                    <c:otherwise>
                        ...
                        <a class="alphabetOnClickLink" href="" onclick="${onClickFunctionName} (
                            <c:forEach var="property" items="${fn:split(onClickFunctionParameters, ',')}">
                                ...
                                '<bean:write name="navigation" property="${property}"/>'
                            </c:forEach>
                        , ${saveRegion}); return false;">${navigation.name}</a><br/>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <tiles:insert definition="roundBorder" flush="false">
                <c:set var="name" value="${phiz:escapeForJS(currentRegion.name, false)}"/>
                ...
                <tiles:put name="data">
                    ...
                    <a class="alphabetOnClickLink" href="#" onclick="${selectFunctionName} (
                        <c:forEach var="property" items="${fn:split(selectFunctionParameters, ',')}">
                            ...
                            '<bean:write name="currentRegion" property="${property}"/>'
                        </c:forEach>
                        , ${saveRegion}); return false;">${currentRegion.name}</a>
                </tiles:put>
            </tiles:insert>
        </c:if>
            ...
            <c:forEach items="${data}" var="element">
                ...
                <c:set var="regName" value="${phiz:replace(regName,'^([г|Г][.]?[ ]+)')}"/>
                ...
                <div class="alphabetItemBlock">
                    ...
                    , ${saveRegion}); return false;">${element.name}</a>
                </div>
