...
import com.rssl.phizicgate.esberibgate.loanclaim.etsm.generated.release_13.*;
 ...
 * @ $Revision$
 */
public class ETSMLoanClaimRequestBuilderRelease13
{
	private static final String ANNUITY_LOAN_PAYMENT = "Annuity";

	private static final String SBOL_APPLICATION_CHANNEL = "8";
	...
	private ApplicationType makeApplication(ETSMLoanClaim claim)
	{
		...
		// 2. Канал продаж - СБОЛ
		application.setChannel(SBOL_APPLICATION_CHANNEL);
	...
	private ApplicantDataType makeApplicant(ETSMLoanClaim claim)
	{
		...
		// (9) Долговые обязательства клиента
		applicant.setLoanList(null);
...
//		passport.setIssuedCode(claimActualPassport.getIssuedCode());
	...
	private MaritalConditionType makeApplicantMaritalCondition(ETSMLoanClaim claim)
	{
		...
		marital.setStatus(claim.getApplicantFamilyStatus().getCode());
	...
	private ApplicantAddDataType makeApplicantAdditionalData(ETSMLoanClaim claim)
	{
		...
		data.setLoanIssue(makeLoanIssue(claim));
	...
	private LoanIssueType makeLoanIssue(ETSMLoanClaim claim)
	{
		...
		// 1. Способ выдачи
		loanIssue.setType(claim.getLoanIssueMethod().getCode().toString());
