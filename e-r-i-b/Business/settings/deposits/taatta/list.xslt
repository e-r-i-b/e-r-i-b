...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		...
		<table class="userTab" cellspacing="0" cellpadding="2" width="100%">
			<tr class="titleTable">
				<td rowspan="2">Cумма вклада</td>
				<td rowspan="2">Валюта вклада</td>
				<td rowspan="2">Срок вклада</td>
				<td colspan="2">Проценты</td>
				<td rowspan="2">Процентная ставка (% годовых)</td>
			</tr>
			<tr class="titleTable">
				<td>Начисление</td>
				<td>Выплата</td>
		...
		<tr>
			...
			<td class="ListItem">
				<xsl:for-each select="$currencies">
					...
					<xsl:variable name="periods" select="$elements_currency/value[@field='calculationPeriod']"/>
					<xsl:for-each select="$periods">
						<xsl:sort data-type="text"/>
						<xsl:if test="position()=1">
							<xsl:choose>
