...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<span class="workspaceTitle" height="40px">Вклады банка</span><hr/>
		<xsl:choose>
			<xsl:when test="count($products)=0">
				<div class="messageTab" align="center">Не найдено ни одного вклада!</div>
			</xsl:when>
			<xsl:otherwise>
    ...
    <xsl:template match="product" mode="first">
        <xsl:if test="not(@departmentId = preceding-sibling::product/@departmentId)">
            ...
            <span class="infoTitle">
                <xsl:value-of select="document('departments.xml')/entity-list/entity[@key = $departmentId]/field[@name = 'name']/text()"/>
            </span>
            <br/>
