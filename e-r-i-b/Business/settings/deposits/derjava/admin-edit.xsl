...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="getDepositList_a" select="document('getDepositList_a.xml')"/>
	...
	<xsl:template match="/">
		    <tr>
				<td colspan="3" class="tblSettingsBeforeInf">
					...
					<tr>
						...
						<td>
							<input type="text" name="field(name)" value="{product/@name}"/>
						</td>
					</tr>
					<tr>
						<td class="Width200 LabelAll">
						...
						<td>
							<textarea id="productDescription" name="field(productDescription)" rows="2" cols="81">
	...
	<xsl:template match="getDepositList_a" mode="list">
		<xsl:for-each select=".//deposit[generate-id(.)=generate-id(key('deposits',string(accountTypeId)))]">
			...
			<tr class="listLine{position() mod 2}">
				...
				<td class="listItem">
					<input id="dep_chk_{$accountTypeId}" name="field(selectedIds)" type="checkbox"
					       value="{$accountTypeId}">
						<xsl:apply-templates
