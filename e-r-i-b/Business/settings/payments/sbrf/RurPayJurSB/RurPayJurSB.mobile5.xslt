...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="adaptDocumentStatus">
        <xsl:call-template name="documentState">
            <xsl:with-param name="state" select="$documentStatus"/>
        </xsl:call-template>
    </xsl:variable>

    <xsl:variable name="isInitialLongOfferState" select="$adaptDocumentStatus = 'INITIAL_LONG_OFFER'"/>
    ...
    <xsl:template match="/form-data" mode="edit">
        ...
        <document>
            ...
            <status><xsl:value-of select="$adaptDocumentStatus"/></status>
                ...
                <paymentDetails>
                    ...
                    <externalFields>
                        ...
                        <xsl:for-each select="$extendedFields">
                            ...
                            <xsl:variable name="editable" select="./IsEditable='true' and (not($isByTemplate) or ./IsMainSum = 'true'
                    ...
                    <xsl:if test="string-length($value)>0">
                        <availableValues>
                            <valueItem>
                                ...
                                <xsl:if test="number($mobileApiVersion) >= 5.20 and string-length($curCode)>0">
