...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:choose>
            <xsl:when test="$mode = 'edit' and $documentState = 'INITIAL'">
            ...
            <xsl:when test="$mode = 'edit' and $documentState = 'INITIAL_LONG_OFFER'">
                <xsl:if test="$app = 'PhizIC'">
                    <xsl:apply-templates mode="PhizIC-edit-long-offer"/>
                </xsl:if>
                <xsl:if test="$app = 'PhizIA'">
                    <xsl:apply-templates mode="PhizIA-edit-long-offer"/>
                </xsl:if>
            </xsl:when>
    ...
    <xsl:template match="/form-data" mode="PhizIC-edit">
        ...
        <script type="text/javascript">
            ...
            var receiverSubTypeButton = receiverSubType;

            function getCommandButtonDelay()
            {
                if ($('#receiverType'))
                {
                    if ($('#receiverType').val() == 'ph')
                    {
                        return 1700;
                    }
                }
    ...
    <xsl:template match="/form-data" mode="PhizIA-edit-long-offer">
        ...
        <xsl:if test="$receiverType = 'several'">
            ...
            <xsl:choose>
                ...
                <xsl:when test="not (starts-with($toResource, 'card:'))">
                    <xsl:variable name="toAccountSelect" select="receiverAccount"/>
                ...
                <xsl:with-param name="rowValue">
                    <b>
                        <span id="reportSource">
                            <xsl:choose>
                                <xsl:when test="starts-with($toResource, 'card:') or fromResourceType='com.rssl.phizic.business.resources.external.CardLink'">
        ...
        <xsl:if test="not(pru:isNewP2PCommissionCalculation())">
            <xsl:call-template name="standartRow">
            ...
            <input type="hidden" name="commissionCurrency"  value="{commissionCurrency}"/>
        </xsl:if>
        ...
        <script type="text/javascript">
            <![CDATA[
                ...
                {
                    eventType = eventType || 'ONCE_IN_WEEK';
                    ...
                    <div>
                        ...
                        <div style="font-size:10pt;">
                            <xsl:choose>
                                ...
                                <xsl:when test="pru:isNewP2PCommissionCalculation()">
                                </xsl:when>
                                <xsl:otherwise>
                                    <span name="commission">
        ...
        <script type="text/javascript">
            ...
            function generateReport1()
            {
                ...
                switch(eventType)
                {
                    case 'ONCE_IN_WEEK':    return 'раз в неделю, по ' + daysOfWeekDesc[(startDate.getDay()+6)%7];
                    case 'ONCE_IN_MONTH':   return 'раз в месяц, ' + startDate.getDate() + '-го числа';
                    case 'ONCE_IN_QUARTER': return 'раз в квартал, ' + startDate.getDate() + '-го числа ' + (startDate.getMonth()%3 + 1) + '-го месяца квартала квартала';
                    case 'ONCE_IN_YEAR':    return 'раз в год, ' + startDate.getDate() + '-го ' + monthsOfYearDesc[startDate.getMonth()];
                }
            }
