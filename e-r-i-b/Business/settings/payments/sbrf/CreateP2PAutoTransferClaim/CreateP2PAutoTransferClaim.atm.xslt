...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:choose>
            <!--Создание заявки на автоперевод (выбор источников списания/зачисления)-->
            <xsl:when test="$mode = 'edit' and $documentStatus = 'INITIAL'">
				<xsl:apply-templates mode="edit"/>
			</xsl:when>
            <!--Создание заявки на автоперевод (ввод периодичности)-->
            <xsl:when test="$mode = 'edit' and $documentStatus = 'INITIAL_LONG_OFFER'">
                <xsl:apply-templates mode="edit-auto-transfer-info"/>
            </xsl:when>
            <!--Просмотр заявки на автоперевод-->
            <xsl:when test="$mode = 'view'">
                <xsl:apply-templates mode="view-auto-transfer-info"/>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="/form-data" mode="edit">
        <initialData>
            ...
            <CreateP2PAutoTransferClaim>
                <xsl:call-template name="initial-claim-data"/>
            </CreateP2PAutoTransferClaim>
        </initialData>
    </xsl:template>

    <xsl:template name="initial-claim-data">
        <receiver>
            <!--тип получателя-->
            <receiverType>
                <xsl:call-template name="receiver-type-list">
                    <xsl:with-param name="name"     select="'receiverType'"/>
                    <xsl:with-param name="title"    select="'Тип перевода'"/>
            ...
            <!--тип получателя-->
            <receiverSubType>
                <xsl:call-template name="receiver-sub-type-list">
                    <xsl:with-param name="name"     select="'receiverSubType'"/>
                    <xsl:with-param name="title"    select="'Подтип перевода'"/>
                    <xsl:with-param name="required" select="true()"/>
                    <xsl:with-param name="editable" select="true()"/>
                    <xsl:with-param name="visible"  select="true()"/>
            ...
            <!--на карту клиенту Сбербанка по номеру карты-->
            <xsl:if test="string-length(receiverName) > 0">
                <name>
                    <xsl:call-template name="stringField">
                        <xsl:with-param name="name"     select="'receiverName'"/>
            ...
            <externalCardNumber>
                <xsl:call-template name="stringField">
                    ...
                    <xsl:with-param name="title"        select="'Номер счета/карты получателя'"/>
                    <xsl:with-param name="required"     select="false()"/>
                    <xsl:with-param name="editable"     select="true()"/>
                    <xsl:with-param name="visible"      select="true()"/>
            ...
            <!--на карту клиенту Сбербанка по номеру телефона-->
            <externalPhoneNumber>
                <xsl:call-template name="integerField">
                    <xsl:with-param name="name"         select="'externalPhoneNumber'"/>
    ...
    <xsl:template match="/form-data" mode="edit-auto-transfer-info">
        <document>
            <form>CreateP2PAutoTransferClaim</form>
            ...
            <CreateP2PAutoTransferClaim>
                ...
                <autoSubDetails>
                    ...
                    <xsl:if test="autoSubType = 'ALWAYS'">
                        <always>
                            <eventType>
                                <xsl:call-template name="auto-sub-event-type-list">
    ...
    <xsl:template match="/form-data" mode="view-auto-transfer-info">
        <document>
            <form>CreateP2PAutoTransferClaim</form>
            <status><xsl:value-of select="$documentStatus"/></status>
            <xsl:if test="string-length($documentId)>0">
    ...
    <xsl:template name="view-claim-data">
            ...
            <xsl:choose>
                <xsl:when test="receiverSubType = 'severalCard'">
                    <toResource>
                        <xsl:call-template name="stringField">
                            <xsl:with-param name="name"     select="'toResource'"/>
                            <xsl:with-param name="title"    select="'Линк карты зачисления'"/>
                            <xsl:with-param name="required" select="true()"/>
                            <xsl:with-param name="editable" select="false()"/>
                            <xsl:with-param name="visible"  select="true()"/>
                ...
                <xsl:when test="receiverSubType = 'ourPhone'">
                    <externalPhoneNumber>
                        <xsl:call-template name="stringField">
                            <xsl:with-param name="name"     select="'externalPhoneNumber'"/>
                            <xsl:with-param name="title"    select="'Номер телефона получателя'"/>
                            <xsl:with-param name="required" select="true()"/>
                            <xsl:with-param name="editable" select="false()"/>
                            <xsl:with-param name="visible"  select="true()"/>
                            <xsl:with-param name="value"    select="externalPhoneNumber"/>
        ...
        <fromResource>
            <xsl:call-template name="stringField">
                <xsl:with-param name="name"     select="'fromResource'"/>
                <xsl:with-param name="title"    select="'Линк счета списания'"/>
                <xsl:with-param name="required" select="true()"/>
                <xsl:with-param name="editable" select="false()"/>
                <xsl:with-param name="visible"  select="true()"/>
