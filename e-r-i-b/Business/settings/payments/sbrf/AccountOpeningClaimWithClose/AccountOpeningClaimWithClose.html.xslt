...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="resources">
        ...
        <xsl:variable name="defaultFunctions" select="'fillCurrency();hideOrShowCourse();'"/>
        <xsl:variable name="isEmptyFrom" select="string-length($activeAccounts) = 0 and $name = 'fromResource'"/>

        <select id="{$name}" name="{$name}">
            <xsl:if test="$isEmptyFrom">
            ...
            <xsl:choose>
                ...
                <xsl:when test="string-length($accountNumber) = 0 and string-length($linkId) = 0">
                    <option value="">Выберите вклад для закрытия</option>
                </xsl:when>
            </xsl:choose>
    ...
    <xsl:template name="loadForm">
        ...
        <xsl:variable name="useDragdealerAccountOpening" select="pu:impliesService('UseDragdealerAccountOpening') and withMinimumBalance = 'true'"/>
        <xsl:variable name="activeAccounts" select="document('possible-closing-with-open-accounts.xml')/entity-list/*"/>
        ...
        <xsl:call-template name="standartRow">
            ...
            <xsl:with-param name="rowValue">
                    ...
                    <input type="hidden" value="0" name="buyAmount" id="buyAmount"/>
            </xsl:with-param>
        </xsl:call-template>
        ...
        <xsl:call-template name="standartRow">
            ...
            <xsl:with-param name="rowValue">
                ...
                <b>&quot;<span id="depositNameText"></span>&quot;</b><span id="depositPromoSign" class="promoCodeLabel"/>
            </xsl:with-param>
            <xsl:with-param name="isAllocate" select="'false'"/>
        </xsl:call-template>
                    ...
                    <xsl:attribute name="onchange">
                        <xsl:choose>
                            <xsl:when test="needInitialFee = 'true'">
                                <xsl:variable name="defaultFunctions" select="'updateMinSummBalance();fillCurrency();setDepositSubType();hideOrShowCourse();'"/>
                                <xsl:choose>
            ...
            <xsl:call-template name="standartRow">
                ...
                <xsl:with-param name="rowValue">
                    <select id="minDepositBalance" name="minDepositBalance"
                            onchange="javascript:setDepositSubType();updateScrollVal();">
                    </select>
                </xsl:with-param>
                <xsl:with-param name="isAllocate" select="'false'"/>
            </xsl:call-template>
        ...
        <xsl:if test="$isAvailableCapitalization = 'true' and $isAvailablePercentPaySchemaChange">
                    ...
                    <xsl:when test="needInitialFee = 'true'">
                        <div class="radioButtonLines">
                            <div class="payments-radioButton">
                                <input class="float" type="radio" value="period" onchange="setPeriodRadioButton(true);" name="closingDateRadio"/>&nbsp;
                                <label class="radioLabel">на</label>&nbsp;
                            </div>
                            <div>
                                <input onchange="setDepositClosingDate();setDepositSubType();updateScrollVal();"
                        ...
                        <div class="radioButtonLines">
                            ...
                            <div>
                                <input readonly="true" onchange="setDepositPeriod();setDepositSubType();updateScrollVal();"
        ...
        <xsl:if test="needInitialFee = 'true' and withMinimumBalance = 'true' and not($useDragdealerAccountOpening)">
           <xsl:call-template name="standartRow">
        ...
        <script type="text/javascript">
            ...
            function addCurrenciesOptions(toResourceCurrency)
            {
                for(var i = 0; i < firstCurrencies.length; i++)
                    if(mapCurrencies[firstCurrencies[i]] != undefined)
                    {
                        addOption(toResourceCurrency, firstCurrencies[i], currencySignMap.get(firstCurrencies[i]), prevCurrencyValue == firstCurrencies[i]);
                        mapCurrencies[firstCurrencies[i]] = false;
                    }
                for(var key in mapCurrencies)
                    if(mapCurrencies[key])
            ...
            function fillDepositCurrency()
            {
                ...
                prevCurrencyValue = '<xsl:value-of select="toResourceCurrency"/>';

                $("#depositNameText").html('<xsl:value-of select="$depositName"/>');
                ...
                addCurrenciesOptions(toResourceCurrency);
            ...
            function setDepositSubType()
            {
                <xsl:choose>
                    <xsl:when test="needInitialFee = 'true' and withMinimumBalance = 'true'">
                        calculateRate(true);
                    </xsl:when>
                    <xsl:otherwise>
                        <!--Сбрасываем предыдущее сообщение об ошибке-->
                        payInput.fieldError('depositName', '');
                        payInput.getParenDivByName('depositName').error = false;
                        payInput.getParenDivByName('depositName').className = payInput.CLASS_NAME;
                        ...
                        <!-- Массив для хранения подвидов по срокам в годах-месяцах-днях -->
                        ...
                        for (var i in balances)
                        {
                            ...
                            if (balance.toPeriod==undefined)
                            {
                                ...
                                {
                                    setElement("depositSubType", balance.subType);
                                    return;
                                }
                            }
                            else
                        ...
                        {
                            setElement("depositSubType", subTypes[0].subType);
                            return;
                        }
                        else
                        {
                            ...
                            {
                                if (days &lt;= parseInt(subTypes[k].toDays, 10) &amp;&amp; days &gt;= parseInt(subTypes[k].fromDays, 10))
                                {
                                    setElement("depositSubType", subTypes[k].subType);
                                    return;
                                }
                            }
                        }
                        ...
                        if (periodData == null || periodData == "")
                            return;
                        payInput.fieldError('depositName', 'Введённый срок вклада не соответствует условиям вклада.<br/>Укажите другой срок вклада. С условиями вклада можно ознакомиться <a
                            id="depositDetailLink" href="#" onclick="showDetails();" class="depositProductInfoLink link">здесь</a>.');
                    </xsl:otherwise>
                </xsl:choose>
            }

            <xsl:if test="needInitialFee = 'true'">
            function updateMinSummBalance(withMinimumBalance)
            {
            ...
            function updateFormByDragDealer(interestRate){
            ...
            function initFormByProduct(onLoad)
            {
                ...
                updateFormByDragDealer(currRate);
    ...
    <xsl:template match="/form-data" mode="view">
        ...
        <xsl:call-template name="standartRow">
            <xsl:with-param name="rowName">
                <xsl:choose>
                    <xsl:when test="state = 'EXECUTED'">Закрыт вклад:</xsl:when>
                    <xsl:otherwise>Закрыть вклад:</xsl:otherwise>
                </xsl:choose>
            </xsl:with-param>
            <xsl:with-param name="rowValue">
                <b>
        ...
        <xsl:call-template name="standartRow">
            ...
            <xsl:with-param name="rowName">Дата закрытия:</xsl:with-param>
            <xsl:with-param name="rowValue">
                <input type="hidden" value="{depoClosingDate}" name="depoClosingDate"/>
                <b><xsl:value-of select="openDate"/></b>
            </xsl:with-param>
            <xsl:with-param name="isAllocate" select="'false'"/>
        </xsl:call-template>
        ...
        <xsl:call-template name="standartRow">
            ...
            <xsl:with-param name="rowName">Дата открытия:</xsl:with-param>
            <xsl:with-param name="rowValue">
                <input type="hidden" value="{openingDate}" name="openingDate"/>
                <b><xsl:value-of select="openDate"/></b>
            </xsl:with-param>
            <xsl:with-param name="isAllocate" select="'false'"/>
        </xsl:call-template>
        ...
        <xsl:choose>
            <xsl:when test="$postConfirmCommission">
                <xsl:call-template name="accountsSumRows">
                    ...
                    <xsl:with-param name="dstCurAmt" select="buyAmount"/>
            ...
            <xsl:otherwise>
                <xsl:call-template name="standartRow">
                    ...
                    <xsl:with-param name="rowValue">
                        <input type="hidden" name="destinationAmount"/>
                        <span class="summ">
        ...
        <xsl:variable name="isConversion" select="not (contains(fromResourceCurrency, toResourceCurrency))"/>
        ...
        <xsl:if test="not($postConfirmCommission) and $isConversion">
           ...
           <xsl:call-template name="standartRow">
                ...
                <xsl:with-param name="rowValue">
                    <input type="hidden" name="chargeOffAmount" id="chargeOffAmount"/>
                    <span class="summ">
        ...
        <script>
            ...
            function init()
            {
                ...
                <xsl:variable name="isNotConversion" select="string-length(sellAmount)=0"/>

                <xsl:if test="$isNotConversion">
            ...
            {
                var termsURL = '<xsl:value-of select="$webRoot"/>/private/deposits/terms.do?id=' + getQueryStringParameter('id');
            ...
            function getDepositInfo()
            {
                ...
                var params = 'id=' + getQueryStringParameter('id') + '&amp;loadInDiv=true';
    ...
    <xsl:template name="fillDepositInfo">
        ...
        <xsl:for-each select="$elementsByTariffDependence">
            ...
            '<xsl:value-of select="minAdditionalFee"/>', indexes, mapCurrencies, prevCurrencyValue, true, '<xsl:value-of select="./segmentCode"/>');
        </xsl:for-each>
    </xsl:template>
	...
	<xsl:template name="employeeState2text">
		...
		<xsl:choose>
            ...
            <xsl:when test="$code='REFUSED'">1Отказан (статус для клиента: "Отклонено банком")</xsl:when>
        ...
        <script type="text/javascript">
            ...
            doOnLoad(function(){
                ...
                horizDrag<xsl:value-of select="$id"/>.animationCallback = function(x, y){
                    ...
                    updateFormByDragDealer(depositRates[inputVal]);
