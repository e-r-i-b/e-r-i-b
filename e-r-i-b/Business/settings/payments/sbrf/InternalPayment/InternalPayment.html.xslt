...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <xsl:when test="string-length($accountNumber) = 0 and string-length($linkId) = 0">
                    <xsl:choose>
                        <xsl:when test="$name = 'fromResource'">
                            <xsl:choose>
                                <xsl:when test="count($activeCards) = 0 and count($activeAccounts) > 0">
                                    <option value="">Выберите счет <xsl:value-of select="$resourceView"/></option>
                                </xsl:when>
                                ...
                                <xsl:when test="count($activeAccounts) + count($activeCards) > 0">
                                    <option value="">Выберите счет/карту <xsl:value-of select="$resourceView"/></option>
                                </xsl:when>
                            </xsl:choose>
                        </xsl:when>
                        <xsl:otherwise>
    ...
    <xsl:template name="resourcesCards">
        ...
        <xsl:for-each select="$activeCards">
            ...
            <!-- Разрешен перевод на основные и дополнительные карты-->
            <xsl:variable name="isToResourceAllowed"   select="$name = 'toResource'"/>
    ...
    <xsl:template match="/form-data" mode="edit-simple-payment">
        ...
        <xsl:variable name="activeNotArrestedCards" select="document('stored-active-cards.xml')/entity-list/*"/>
        ...
        <xsl:if test="$isTemplate != 'true'">
            &nbsp;
            <span class="simpleLink" onclick="javascript:openTemplateList('INTERNAL_TRANSFER');">
       		...
       		function showHideOperationTypeRow()
       		{
                ...
                if (toResource != "" &amp;&amp; fromResource != "")
                {
                    ...
                    else //валюты равны ->1 сумма (списания)
                    {
                        ...
                        ensureElement("exactAmount").value  = "charge-off-field-exact";
                    }
                }
    ...
    <xsl:template match="/form-data" mode="edit-long-offer">
        ...
        <xsl:variable name="cards" select="document('stored-active-or-arrested-cards.xml')/entity-list/*"/>
            ...
            <![CDATA[
                ...
                function refreshSumType(event, sumType)
                {
                    ...
                    if (isPeriodic(event) || event == "BY_ANY_RECEIPT")
                    {
                        ...
                        //данные типы только для перевода карта-вклад
                        if (fromResource.indexOf('card:') != -1 && toResource.indexOf('account:') != -1)
    ...
    <xsl:template match="/form-data" mode="view-simple-payment">
        ...
        <xsl:if test="fromResourceType='com.rssl.phizic.business.resources.external.CardLink' and toResourceType='com.rssl.phizic.business.resources.external.CardLink' and fromResourceCurrency!=toResourceCurrency">
            <script type="text/javascript">
            ...
            <xsl:otherwise>
                <xsl:if test="$isChargeOff or ($isConversion and not($longOffer))">
                    <xsl:call-template name="standartRow">
                        ...
                        <xsl:with-param name="rowName">
                            <xsl:choose>
                                <xsl:when test="$isConversion and not($longOffer) or fromResourceCurrency!=toResourceCurrency">
                        ...
                        <xsl:with-param name="rowValue">
                            ...
                            <xsl:if test="not ($longOffer) or (isSumModify != 'true' and $longOffer)">
                                <xsl:if test="string-length(sellAmount)>0">
                                     <span>
        ...
        <xsl:if test="$isTemplate!='true'">
            <xsl:choose>
                ...
                <xsl:when test="state = 'SAVED' and not(fromResourceType='com.rssl.phizic.business.resources.external.CardLink' and toResourceType='com.rssl.phizic.business.resources.external.CardLink') and (not($postConfirmCommission) or ($postConfirmCommission and string-length(commission)=0))">
                    <script type="text/javascript">
                    <![CDATA[
                        if(window.addMessage != undefined)
