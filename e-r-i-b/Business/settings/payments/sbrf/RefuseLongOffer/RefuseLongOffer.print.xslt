...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/form-data" mode="printCheck">
        ...
        <xsl:call-template name="paramsCheck">
            ...
            <xsl:with-param name="value">
                <xsl:for-each select="$extendedFields">
                    ...
                    <xsl:variable name="hidden" select="./field[@name='hidden']='true'"/>
                    <xsl:variable name="readonly" select="./field[@name='readonly']='true'"/>
                    <xsl:variable name="type" select="./field[@name='type']"/>
                    <xsl:variable name="id" select="./@key"/>

                    <xsl:if test="not($hidden and $readonly) and not($type='calendar')">
                         <br/>
                         <xsl:value-of select="$name"/>:&nbsp;<xsl:value-of select="$formData/*[name()=$id]/text()"/>
                    </xsl:if>
        ...
        <xsl:call-template name="paramsCheck">
            ...
            <xsl:with-param name="value"><xsl:value-of  select="nameService"/></xsl:with-param>
        </xsl:call-template>
        <br/>
        <xsl:call-template name="paramsCheck">
            <xsl:with-param name="title">получатель платежа</xsl:with-param>
            <xsl:with-param name="value"><xsl:value-of  select="receiverName"/></xsl:with-param>
        </xsl:call-template>
        <br/>
      ...
      <xsl:template name="stamp">
          ...
          <div id="stamp" class="stamp">
             ...
             <xsl:choose>
                 ...
                 <xsl:when test="state='DISPATCHED' or state='ERROR'">          
