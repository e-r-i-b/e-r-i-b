...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="state2text">
		...
		<xsl:choose>
            ...
            <xsl:when test="$code='SEND'">Обрабатывается</xsl:when>
			<xsl:when test="$code='EXECUTED'">Исполнен</xsl:when>
			<xsl:when test="$code='REFUSED'">Отказан</xsl:when>
			<xsl:when test="$code='RECALLED'">Отозван</xsl:when>
			<xsl:when test="$code='ERROR'">Приостановлен</xsl:when>
            <xsl:when test="$code='PARTLY_EXECUTED'">Обрабатывается</xsl:when>
		</xsl:choose>
	</xsl:template>
