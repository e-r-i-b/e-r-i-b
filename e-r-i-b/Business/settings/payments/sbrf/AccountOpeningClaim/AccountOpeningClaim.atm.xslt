...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="xalan mask mu pu au ph dph sh">
        ...
        <xsl:choose>
            <xsl:when test="string-length($groupToShow) > 0">
                <xsl:call-template name="groupFilteredElements">
                    <xsl:with-param name="depositDescriptions" select="$fullDepositDescriptions/options/element[group/groupCode = $groupToShow]"/>
    ...
    <xsl:template name="groupFilteredElements">
        ...
        <xsl:if test="string-length(atmPlace)>0">
            <atmPlace>
                <xsl:call-template name="stringField">
                    <xsl:with-param name="name">atmPlace</xsl:with-param>
                    <xsl:with-param name="title">Место установки устройства</xsl:with-param>
                    <xsl:with-param name="required" select="true()"/>
                    <xsl:with-param name="editable" select="false()"/>
                    <xsl:with-param name="visible" select="false()"/>
                    <xsl:with-param name="value">
                        <xsl:choose>
                            <xsl:when test="(atmPlace = 'FILIAL') or (atmPlace = 'PREDPRIYATIE')">
                                <xsl:value-of select="atmPlace"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="0"/>
                            </xsl:otherwise>
        ...
        <xsl:if test="string-length(atmPlace)>0">
            <atmTB>
                <xsl:call-template name="stringField">
                    <xsl:with-param name="name">atmTB</xsl:with-param>
                    <xsl:with-param name="title">Номер ТБ</xsl:with-param>
                    <xsl:with-param name="required" select="true()"/>
                    <xsl:with-param name="editable" select="false()"/>
                    <xsl:with-param name="visible" select="false()"/>
                    <xsl:with-param name="value" select="atmTB"/>
                </xsl:call-template>
            </atmTB>
        </xsl:if>
        ...
        <xsl:if test="string-length(atmPlace)>0">
            <atmOSB>
                <xsl:call-template name="stringField">
                    <xsl:with-param name="name">atmOSB</xsl:with-param>
                    <xsl:with-param name="title">Номер ОСБ</xsl:with-param>
                    <xsl:with-param name="required" select="true()"/>
                    <xsl:with-param name="editable" select="false()"/>
                    <xsl:with-param name="visible" select="false()"/>
                    <xsl:with-param name="value" select="atmOSB"/>
                </xsl:call-template>
            </atmOSB>
        </xsl:if>
        ...
        <xsl:if test="string-length(atmPlace)>0">
            <atmVSP>
                <xsl:call-template name="stringField">
                    <xsl:with-param name="name">atmVSP</xsl:with-param>
                    <xsl:with-param name="title">Номер ВСП</xsl:with-param>
                    <xsl:with-param name="required" select="true()"/>
                    <xsl:with-param name="editable" select="false()"/>
                    <xsl:with-param name="visible" select="false()"/>
                    <xsl:with-param name="value" select="atmVSP"/>
                </xsl:call-template>
            </atmVSP>
        </xsl:if>
        ...
        <depositGroup>
            <xsl:call-template name="stringField">
                <xsl:with-param name="name">depositGroup</xsl:with-param>
                <xsl:with-param name="title">Группа вкладного продукта</xsl:with-param>
                <xsl:with-param name="required" select="true()"/>
                <xsl:with-param name="editable" select="false()"/>
                <xsl:with-param name="visible" select="false()"/>
                <xsl:with-param name="value" select="depositGroup"/>
            </xsl:call-template>
        </depositGroup>
        ...
        <minAdditionalFee>
            <xsl:call-template name="moneyField">
                ...
                <xsl:with-param name="required" select="needInitialFee = 'true'"/>
                <xsl:with-param name="editable" select="true()"/>
                <xsl:with-param name="visible" select="false()"/>
    ...
    <xsl:template name="resourceField">
        ...
        <xsl:param name="activeAccounts"   select="''"/>
        <xsl:param name="activeCards"      select="''"/>
        <xsl:param name="linkId"           select="''"/>
        <xsl:param name="selectedResource" select="''"/>
        <xsl:param name="validators"       select="''"/>
        <xsl:param name="displayedValue"   select="''"/>
	    <xsl:param name="cardsForPercent"  select="''"/>
