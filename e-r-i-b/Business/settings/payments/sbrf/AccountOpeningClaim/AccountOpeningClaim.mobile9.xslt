...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="initialData">
        ...
        <xsl:variable name="depositDescriptions" select="$fullDepositDescriptions/options/element[group/groupCode = $code and (segmentCode = 0 or segmentCode = 1)]"/>

        <xsl:variable name="clientTariff">
            <xsl:choose>
                <xsl:when test="$tariffPlanCode=0 or count($depositDescriptions[tariffPlanCode=$tariffPlanCode]) > 0">
        ...
        <xsl:variable name="elements" select="$depositDescriptions[(0!=$clientTariff and tariffPlanCode=$tariffPlanCode) or (0=$clientTariff and tariffPlanCode!=1 and tariffPlanCode!=2 and tariffPlanCode!=3)]"/>
                ...
                <xsl:with-param name="value">
                    ...
                    <xsl:choose>
                        <xsl:when test="isPension = 'true'">
                            <xsl:choose>
                                <xsl:when test="($code = '' or $code = '-22' or $code = '0') and count($pensionElements) &gt; 0 ">
                                    <xsl:value-of select="'true'"/>
                                </xsl:when>
                                <xsl:when test="($code != '' and $code != '-22' and $code != '0') and count($pensionElements_param) &gt; 0 ">
                                    <xsl:value-of select="'true'"/>
                                </xsl:when>
                                <xsl:otherwise>
