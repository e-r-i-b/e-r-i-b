...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/form-data" mode="view">
        ...
        <xsl:call-template name="standartRow">
            ...
            <xsl:with-param name="required" select="$mode = 'edit'"/>
			<xsl:with-param name="rowValue">
                ...
                <xsl:if test="$mode = 'edit'">
                    ...
                    <xsl:variable name="enteredCard" select="document('entered-cards.xml')/entity-list/*"/>
                    <xsl:call-template name="resources">
                        ...
                        <xsl:with-param name="enteredCard" select="$enteredCard"/>
                    </xsl:call-template>
               </xsl:if>
			</xsl:with-param>
		</xsl:call-template>
    ...
    <xsl:template match="/form-data" mode="edit-auto-sub-info">
        ...
        <xsl:call-template name="standartRow">
            ...
            <xsl:with-param name="rowValue">
                ...
                <input type="hidden" name="autoSubType" id="autoSubType" value="{$autoSubType}"/>
            </xsl:with-param>
        </xsl:call-template>
...
<xsl:template name="resources">
    ...
    <xsl:param name="enteredCard"/>

    <input name="initialCard" type="hidden" value="{initialCard}"/>
    <xsl:variable name="initialCard" select="initialCard"/>

    <select id="{$name}" name="{$name}" class="paymentCutWidthSelect">
        <xsl:choose>
                ...
                <xsl:choose>
                    <xsl:when test="$loginedFromEmployeeByCard = 'true'">
                        <xsl:for-each select="$activeCards">
                            ...
                            <xsl:for-each select="$enteredCard">
                                <xsl:variable name="enteredNode" select="."/>
                                <xsl:variable name="cardType" select="$enteredNode/field[@name='cardType']"/>
                                <xsl:if test="($enteredNode/@key = $curNode/@key and $enteredNode/@key!=$accountNumber and $enteredNode/field[@name='code']/text() != $linkId)">
                                    <xsl:if test="$cardType!='credit' or ($cardType ='credit' and spu:isCreditCardSupported(recipient))">
                                        <xsl:call-template name="showCard">
                                            <xsl:with-param name="accountNumber" select="$accountNumber"/>
                                            <xsl:with-param name="card"          select="$curNode"/>
                                            <xsl:with-param name="linkId"        select="$linkId"/>
                                        </xsl:call-template>
                                    </xsl:if>
                                </xsl:if>
                            </xsl:for-each>
                            <xsl:if test="$curNode/@key = $accountNumber or $curNode/field[@name='code']/text() = $linkId or $curNode/field[@name='code']/text()=$initialCard">
                                <xsl:call-template name="showCard">
                                    <xsl:with-param name="accountNumber" select="$accountNumber"/>
                                    <xsl:with-param name="card"          select="$curNode"/>
                                    <xsl:with-param name="linkId"        select="$linkId"/>
...
<xsl:template name="showCard">
    ...
    <option>
        <xsl:attribute name="value">
            <xsl:value-of select="$card/field[@name='code']/text()"/>
        </xsl:attribute>
        <xsl:if test="$accountNumber=$card/@key or $linkId=$card/field[@name='code']/text()">
            <xsl:attribute name="selected"/>
        </xsl:if>
        <xsl:value-of select="mask:getCutCardNumber($card/@key)"/>
        &nbsp;
        [<xsl:value-of select="$card/field[@name='description']"/>]
