...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="tableRow" match="/list-data/entity-list/entity">
		<xsl:element name="tr">
			...
			<td class="listItem" align="center">
				&nbsp;
				<xsl:if test="attributes.type.value = '2'">
					Покупка
				</xsl:if>
				<xsl:if test="attributes.type.value = '1'">
					Продажа
				</xsl:if>
				&nbsp;
			</td>
			...
			<td class="listItem" align="right" nowrap="true">
				&nbsp;
				<xsl:if test="attributes.type.value = '2'">
					<xsl:value-of select="format-number(attributes.rur-amount.value, '#0.00')"/>&nbsp;
