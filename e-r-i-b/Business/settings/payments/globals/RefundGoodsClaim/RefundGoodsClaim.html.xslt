...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/form-data" mode="view">
		...
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Дата возврата:</xsl:with-param>
			<xsl:with-param name="rowValue"><b><xsl:value-of select="documentDate"/></b></xsl:with-param>
		</xsl:call-template>

		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Номер документа оплаты заказа:</xsl:with-param>
			<xsl:with-param name="rowValue"><b><xsl:value-of select="recalledDocumentNumber"/></b></xsl:with-param>
		</xsl:call-template>

		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Сумма возврата:</xsl:with-param>
        ...
        <xsl:if test="recalledDocumentFormName = 'ExternalProviderPayment'">
            <xsl:call-template name="standartRow">
                <xsl:with-param name="rowName">Список возвращенных товаров:</xsl:with-param>
                <xsl:with-param name="rowValue">
                    <div class="goodsList">
                    ...
                    <xsl:if test="count($internetOrderFields)>0">
                        <table id="details">
                            <tr class="internetOrderTabHead">
                                ...
                                <th class="internetOrderTabHeadSecond">Сумма возврата</th>
                                <th>&nbsp;</th>
                            </tr>
		...
		<xsl:if test="recalledDocumentFormName = 'AirlineReservationPayment'">
            <xsl:call-template name="standartRow">
                <xsl:with-param name="rowName">Список возвращенных билетов:</xsl:with-param>
                <xsl:with-param name="rowValue">
                    <div class="goodsList">
                    <xsl:variable name="internetOrderFields" select="document('returnedGoodsDictionary.xml')/tickets/ticket"/>
                    <xsl:if test="count($internetOrderFields)>0">
                        <table id="details">
                            <tr class="internetOrderTabHead">
                                <th class="internetOrderTabHeadFirst">Номер билета</th>
                                <th class="internetOrderTabHeadSecond">Сумма возврата</th>
                                <th>&nbsp;</th>
                            </tr>
                            <xsl:for-each select="$internetOrderFields">
