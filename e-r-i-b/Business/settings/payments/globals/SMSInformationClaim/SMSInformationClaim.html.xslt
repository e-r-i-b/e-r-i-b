...
<xsl:variable name="formData" select="/form-data"/>
    ...
    <xsl:template name="editHTML">
	    ...
	    <xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Сотовый оператор</xsl:with-param>
			<xsl:with-param name="rowValue">
				<select id="operatorSelect" name="operatorSelect">
					<option value='МТС'>МТС</option>
					<option value='БиЛайн'>БиЛайн</option>
					<option value='Мегафон'>Мегафон</option>
					<option value='Скайлинк'>Скайлинк</option>
				</select>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Номер телефона (10 цифр)</xsl:with-param>
			<xsl:with-param name="rowValue">
				<input id="phoneCode" name="phoneCode" type="text" size="3" maxlength="3">
					<xsl:attribute name="value">
							<xsl:value-of select="phoneCode"/>
						</xsl:attribute>
					</input>-
					<input id="phoneNumber" name="phoneNumber" type="text" size="15" maxlength="7">
						<xsl:attribute name="value">
	...
	<xsl:template name="initCardsScript">
     ...
     <xsl:template match="/form-data" mode="view">
        ...
        <xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Сотовый оператор</xsl:with-param>
			<xsl:with-param name="rowValue">
				<xsl:value-of select="operatorSelect"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Телефон</xsl:with-param>
			<xsl:with-param name="rowValue">
				<xsl:value-of select="phoneCode"/> - <xsl:value-of select="phoneNumber"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
...
<metaInformation>
<scenarios ><scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="test.xml" htmlbaseurl="" outputurl="" processortype="internal" useresolver="yes" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator=""/><scenario default="no" name="Scenario2" userelativepaths="yes" externalpreview="no" url="..\SMSInformationClaim" htmlbaseurl="" outputurl="" processortype="internal" useresolver="yes" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator=""/></scenarios><MapperMetaTag><MapperInfo srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/><MapperBlockPosition></MapperBlockPosition><TemplateContext></TemplateContext><MapperFilter side="source"></MapperFilter></MapperMetaTag>
</metaInformation>
-->