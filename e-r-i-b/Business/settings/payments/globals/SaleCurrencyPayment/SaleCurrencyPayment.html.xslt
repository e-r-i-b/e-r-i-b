...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<input id="foreignCurrency" name="foreignCurrency" type="hidden"/>
				<tr>
					...
					<td>
						...
						<xsl:if test="$personAvailable">
							<select id="payerAccountSelect" name="payerAccountSelect" onchange="javascript:updateCurrencyCode(); showHideOperationCode();">
				...
				<tr>
					...
					<td>
						...
						<xsl:if test="$personAvailable">
							<select id="receiverAccountSelect" name="receiverAccountSelect" onchange="javascript:showHideOperationCode();">
	...
	<xsl:template match="/form-data" mode="view">
			...
			<xsl:if test="foreignCurrencyAmount != '0.00'">
				<tr>
					<td class="Width120 LabelAll" nowrap="true">Сумма в иностранной валюте</td>
					<td>
	...
	<xsl:template name="state2text">
		<xsl:param name="code"/>
		<xsl:choose>
			<xsl:when test="$code='SAVE'">Введен</xsl:when>
