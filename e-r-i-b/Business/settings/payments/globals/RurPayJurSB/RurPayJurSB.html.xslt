...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
			...
			<tr>
				<td>
					...
					<xsl:for-each select="$extendedFields">
						...
						<tr id='tr_{$id}' style="padding-top:2px; padding-bottom:2px">
							<td class="Width120 LabelAll" style="font-size:7.5pt; width:140px; vertical-align:middle">
								<script type="text\javascript">
									document.write('<xsl:value-of select="./field[@name='name']"/>'.toUpperCase());
								</script>
							</td>
							<td style="vertical-align:middle">
								...
								<xsl:variable name="sum" select="./field[@name='mainSum']"/>

								<xsl:if test="./field[@name='visible']='true'"></xsl:if>
								<xsl:choose>
									<!--todo: Странный пустой when-->
									...
									<xsl:otherwise>
										<xsl:if test="$sum='true'">
											...
											<input type="text" value="{$val}" id="{$id}" size="{$length}" name="{$id}" onChange="setSum('{$id}')"/>
										</xsl:if>
										<xsl:if test="$sum='false'">
											<input type="text" value="{$val}" id="{$id}" size="{$length}" name="{$id}" />
				...
				<td>
					...
					<script type="text/javascript">
						...
						if(payPeriod !='' &amp;&amp; payPeriod.split(';').length == 2)
						{
							var payMonth = payPeriod.split(';')[1].split('.')[0];
							var payYear = payPeriod.split(';')[1].split('.')[1];
						    document.getElementById('hiddenYear').value = payYear;
						    setPayPeriod(payMonth);
						}
	...
	<xsl:template match="/form-data" mode="view">
			...
			<tr>
				...
				<td>
					<xsl:value-of select="amount"/>&nbsp;<xsl:value-of select="amountCurrency"/> &nbsp;&nbsp;Комиссия:&nbsp;<xsl:value-of select="commission"/>&nbsp;<xsl:value-of select="amountCurrency"/></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
                    ...
                    [<xsl:value-of select="receiverCorAccountType"/>]
				    <xsl:value-of select="amountCurrency"/>
				</td>
			</tr>
			<xsl:if test="appointment != 'gorod'">
