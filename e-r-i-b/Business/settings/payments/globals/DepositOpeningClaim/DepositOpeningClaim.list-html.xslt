...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="tableRow" match="/list-data/entity-list/entity">
		<tr>
			...
			<td class="listItem" nowrap="true">
				&nbsp;
				<xsl:variable name="claimType" select="formName"/>
				<xsl:if test="$claimType = 'DepositOpeningClaim'">Открытие вклада</xsl:if>
				<xsl:if test="$claimType = 'AccountOpeningClaim'">Открытие счета</xsl:if>
				&nbsp;
			</td>
			<td class="listItem">
			...
			<xsl:if test="state.code='ACCEPTED'">
				<script type="text/javascript">
	...
	<xsl:template name="state2text">
		...
		<xsl:choose>
			...
			<xsl:when test="$code='SENDED'">Принят</xsl:when>
