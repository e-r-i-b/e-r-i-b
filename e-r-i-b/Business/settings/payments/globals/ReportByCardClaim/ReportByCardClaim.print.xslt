	...
	<xsl:template match="/form-data" mode="view">
        ...
        <table cellpadding="0" cellspacing="0">
            <tr>
                ...
                <td><xsl:value-of select="report-card-name"/>&nbsp;<xsl:value-of select="mask:getCutCardNumber(report-card-number)"/></td>
            </tr>
            <tr>
                <td>E-mail</td>
                ...
                <td>
                    <xsl:choose>
                        <xsl:when test="report-order-type = 'IR'">
                            <xsl:value-of select="report-start-date-formated"/> &#8212; <xsl:value-of select="report-end-date-formated"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="report-start-date-formated"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </td>
            </tr>
            <tr>
                <td>Язык отчета</td>
                <td><xsl:value-of select="report-lang"/></td>
            </tr>
            <tr>
                <td>Формат отчета</td>
                <td><xsl:value-of select="report-format"/></td>
            </tr>
            <tr>
                <td>Статус заявки</td>
                <td>
