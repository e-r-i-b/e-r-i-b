...
<xsl:variable name="currentPerson" select="document('currentPerson.xml')/entity-list"/>
		...
		<xsl:call-template name="standartRow">
			...
			<xsl:with-param name="rowValue">
				<select id="currency" name="currency">
					 <option value="российские рубли">Рубли РФ</option>
					 <option value="доллары США">Доллары США</option>
		...
		<xsl:call-template name="standartRow">
			...
			<xsl:with-param name="rowValue">
				<input type="checkbox" name="cardType" id="cardType"/>&nbsp;Основная
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Услуга SMS-информирования</xsl:with-param>
			<xsl:with-param name="rowValue">
		...
		{
			var text = "<table><tr><td class='Width120 LabelAll'>Дизайн</td>"
						+"<td width='78%'><select id='selectDesign' name='selectDesign'>"
						+"<option value='Обычный'>Обычный</option>"
						+"<option value='Лига Чемпионов'>Лига Чемпионов</option></select></td>";
		...
		function refreshDesign()
		{
			...
			{
			    updateTagText("hideLayer", "<input id='selectDesign' name='selectDesign' type='hidden' value='Обычный'>");
			}
		}
...
<xsl:template match="/form-data" mode="view">
	...
	<xsl:variable name="offices" select="document('departments.xml')"/>
	    ...
	    <xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Дизайн карты</xsl:with-param>
			<xsl:with-param name="rowValue">
				<xsl:value-of select="selectDesign"/>
			</xsl:with-param>
		</xsl:call-template>
	    <xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Валюта пластиковой карты</xsl:with-param>
			<xsl:with-param name="rowValue">
