    ...
    <xsl:template match="/form-data" mode="printDocumentCheck">
        <DocumentCheck>
            ...
            <LoanProductDocumentCheck>
                ...
                <creditType>
                    <xsl:call-template name="stringField">
                        <xsl:with-param name="name">loanProductName</xsl:with-param>
                        <xsl:with-param name="title">Наименование кредита</xsl:with-param>
                        <xsl:with-param name="required" select="true()"/>
                        <xsl:with-param name="editable" select="false()"/>
                        <xsl:with-param name="visible" select="true()"/>
                        <xsl:with-param name="changed" select="false()"/>
                        <xsl:with-param name="value" select="loanProductName"/>
                    </xsl:call-template>
                </creditType>
                ...
                <sum>
                    <amount>
                        <xsl:call-template name="moneyField">
                            <xsl:with-param name="name">loanAmount</xsl:with-param>
                            <xsl:with-param name="title">Сумма</xsl:with-param>
                            <xsl:with-param name="required" select="true()"/>
                            <xsl:with-param name="editable" select="false()"/>
                            <xsl:with-param name="visible" select="true()"/>
                            <xsl:with-param name="changed" select="false()"/>
                            <xsl:with-param name="value" select="loanAmount"/>
                        </xsl:call-template>
                    </amount>
                    <currency>
                        <xsl:call-template name="stringField">
                            <xsl:with-param name="name">loanCurrency</xsl:with-param>
                            <xsl:with-param name="title">Валюта</xsl:with-param>
                            <xsl:with-param name="required" select="true()"/>
                            <xsl:with-param name="editable" select="false()"/>
                            <xsl:with-param name="visible" select="true()"/>
                            <xsl:with-param name="changed" select="false()"/>
                            <xsl:with-param name="value" select="loanCurrency"/>
                        </xsl:call-template>
                    </currency>
                </sum>
                ...
                <duration>
                    <xsl:call-template name="integerField">
                        <xsl:with-param name="name">loanPeriod</xsl:with-param>
                        <xsl:with-param name="title">Срок в месяцах</xsl:with-param>
                        <xsl:with-param name="required" select="true()"/>
                        <xsl:with-param name="editable" select="false()"/>
                        <xsl:with-param name="visible" select="true()"/>
                        <xsl:with-param name="changed" select="false()"/>
                        <xsl:with-param name="value" select="loanPeriod"/>
                    </xsl:call-template>
                </duration>
                ...
                <mobilePhone>
                    <xsl:call-template name="stringField">
                        <xsl:with-param name="name">mobileNumber</xsl:with-param>
                        <xsl:with-param name="title">Мобильный телефон</xsl:with-param>
                        <xsl:with-param name="required" select="true()"/>
                        <xsl:with-param name="editable" select="false()"/>
                        <xsl:with-param name="visible" select="true()"/>
                        <xsl:with-param name="changed" select="false()"/>
                        <xsl:with-param name="value" select="mobileNumber"/>
                    </xsl:call-template>
                </mobilePhone>

                <hirer>
                    <xsl:call-template name="stringField">
                        <xsl:with-param name="name">companyFullName</xsl:with-param>
                        <xsl:with-param name="title">Работодатель</xsl:with-param>
                        <xsl:with-param name="required" select="true()"/>
                        <xsl:with-param name="editable" select="false()"/>
                        <xsl:with-param name="visible" select="true()"/>
                        <xsl:with-param name="changed" select="false()"/>
                        <xsl:with-param name="value" select="companyFullName"/>
                    </xsl:call-template>
                </hirer>

                <averageIncomePerMonth>
                    <xsl:call-template name="moneyField">
                        <xsl:with-param name="name">basicIncome</xsl:with-param>
                        <xsl:with-param name="title">Средний доход в месяц</xsl:with-param>
                        <xsl:with-param name="required" select="true()"/>
                        <xsl:with-param name="editable" select="false()"/>
                        <xsl:with-param name="visible" select="true()"/>
                        <xsl:with-param name="changed" select="false()"/>
                        <xsl:with-param name="value" select="basicIncome"/>
                    </xsl:call-template>
                </averageIncomePerMonth>
                ...
                <getPaidOnAccount>
                    <xsl:call-template name="booleanField">
                        ...
                        <xsl:with-param name="visible" select="true()"/>
                        <xsl:with-param name="changed" select="false()"/>
                        <xsl:with-param name="value" select="sbrfRelationType = 'GET_PAID_ON_SBRF_ACCOUNT'"/>
                    </xsl:call-template>
                </getPaidOnAccount>
