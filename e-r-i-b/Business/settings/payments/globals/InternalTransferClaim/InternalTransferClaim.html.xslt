...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="editHtml">
		...
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Вклад для снятия</xsl:with-param>
			<xsl:with-param name="rowValue">
		...
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Неснижаемый остаток вклада</xsl:with-param>
			<xsl:with-param name="rowValue">
				<span id="minBalanceVal" name="minBalanceVal"/>&nbsp;<span id="minBalanceCurrency"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Cумма для снятия</xsl:with-param>
			<xsl:with-param name="rowValue">
				<input type="text" id="amount" name="amount"/>&nbsp;<span id="amountCurrency"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Перевести на счет</xsl:with-param>
			<xsl:with-param name="rowValue">
	...
	<xsl:template name="editInitControls">
		...
		updateSelect("deposit", createOptions(deposits, depositOptionCreator, "Нет доступных вкладов", "Выберите вклад для снятия"));
	...
	<xsl:template name="viewHtml" mode="view" match="form-data">
		...
		<xsl:if test="not($adDate = '')">
			<xsl:call-template name="standartRow">
				<xsl:with-param name="rowName">Дата приема заявки</xsl:with-param>
				<xsl:with-param name="rowValue">
		...
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Cумма списания</xsl:with-param>
			<xsl:with-param name="rowValue">
