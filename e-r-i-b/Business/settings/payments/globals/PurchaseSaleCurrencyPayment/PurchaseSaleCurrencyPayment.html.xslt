	...
	<xsl:template match="/form-data" mode="edit">
		...
		<xsl:call-template name="standartRow">
			...
			<xsl:with-param name="rowValue">
                ...
                <xsl:variable name="foreignCurrencyAccounts"
									  select="document('active-foreign-currency-accounts.xml')/entity-list/*"/>
				<select id="currencyCodeSelect" name="foreignCurrency"
				  ...
				  onkeyup="javascript:setCurrencyCode()">
                    ...
                    <xsl:if test="count($foreignCurrencyAccounts) = 0">
		...
		<script type="text/javascript">
			var currencyCodesDescriptions = new Array();
			var currencies = new Array();
			currencies[''] = 0;
            currencyCodesDescriptions['']=0;
			...
			<xsl:for-each select="$currency">
				rateAr = new Array();
				<xsl:variable name="currencyDescription" select="@description"/>
                <xsl:for-each select="*">

				    rateAr['<xsl:value-of select="@name"/>'] = '<xsl:value-of select="./text()"/>';
                </xsl:for-each>
                rateAr['DESCRIPTION'] = '<xsl:value-of select="$currencyDescription"/>';
        ...
        // заполняет комбо-бокс с валютами.
        function fillCurrenciesComboBox()
        {
			var currencySelectElem = document.getElementById("currencyCodeSelect");
            var code = document.getElementById("foreignCurrency");
			for ( var i = 0; i < accounts.length; i++ )
            {
               var currentCurrency = accounts[i].currency;
               var alreadyInArray = false; //есть ли такая валюта в массиве кодов и описаний валют
               for ( var j = 0; j < currencyCodesDescriptions.length; j++)
               {
                    if (currencyCodesDescriptions[j].code == currentCurrency)
                    {
                        alreadyInArray = true; break;
                    }
               }
               if ( alreadyInArray == false )
               {
                    ...
                    cur.description = currencies[currentCurrency]['DESCRIPTION'];
                    currencyCodesDescriptions[currencyCodesDescriptions.length] = cur;
					currencySelectElem.options[currencySelectElem.options.length] = new Option(cur.description, cur.code);
                    if(cur.code == code)
							currencySelectElem.options[currencySelectElem.options.length-1].selected = true;
				}
            }

        }

        //эта функция заполняет комбо-бокс с валютными счетами
			...
			<xsl:call-template name="standartRow">
				...
				<xsl:with-param name="rowValue">
					<xsl:choose>
						<xsl:when test="type = 'BUY'">
                            <xsl:choose>
                                <xsl:when test="$amountChanged='true'">
                                    <b><xsl:value-of select="chargeOffAmount"/>&nbsp;</b>
                                </xsl:when>
						...
						<xsl:when test="type = 'SALE'">
                            <xsl:choose>
                                 <xsl:when test="$amountChanged='true'">
                                     <b><xsl:value-of select="destinationAmount"/>&nbsp;</b>
                                 </xsl:when>
                                 <xsl:otherwise><xsl:value-of select="destinationAmount"/>&nbsp;</xsl:otherwise>
                             </xsl:choose>
						</xsl:when>
					</xsl:choose>
