...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="editHtml">
		<xsl:call-template name="standartRow">
			...
			<xsl:with-param name="rowValue">
				<input type="text" id="documentNumber" name="documentNumber" disabled="true"/>
			</xsl:with-param>
		</xsl:call-template>
	    <xsl:call-template name="standartRow">
	    ...
	    <xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Вклад для пополнения</xsl:with-param>
			<xsl:with-param name="rowValue">
		...
		<xsl:call-template name="standartRow">
			...
			<xsl:with-param name="rowValue">
				<input type="text" id="contractNumber" name="contractNumber" disabled="true"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Дата открытия</xsl:with-param>
			<xsl:with-param name="rowValue">
				<input type="text" id="openingDate" name="openingDate" disabled="true"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Дата окончания</xsl:with-param>
			<xsl:with-param name="rowValue">
				<input type="text" id="finishingDate" name="finishingDate" disabled="true"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="required" select="'false'"/>
			<xsl:with-param name="rowName">Минимальная сумма доп.взноса</xsl:with-param>
			<xsl:with-param name="rowValue">
				<input type="text" id="minAmount" disabled="true"/>&nbsp;<span id="minAmountCurrency"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Cумма доп.взноса</xsl:with-param>
			<xsl:with-param name="rowValue">
		...
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Пополнить со счета</xsl:with-param>
			<xsl:with-param name="rowValue">
		...
		function refreshTexts()
		{
			...
			if (depositSelect.selectedIndex < 0)
				depositSelect.selectedIndex = 0;
			...
			setValue("minAmount", depositObject.minAmount);
			setValue("minAmountCurrency", depositObject.currency);
		...
		function refreshDestinationAccounts(deposit){
			...
			updateSelect("account", createOptions(accounts, accountsOptionCreator, "Нет доступных счетов", "Выберите счет"));
		}
	...
	<xsl:template name="editInitControls">
		...
		updateSelect("deposit", createOptions(deposits, depositOptionCreator, "Нет доступных вкладов"));
	...
	<xsl:template name="initDepositsScript">
		<script type="text/javascript">
			...
			<xsl:if test="$isOpen='true' and @is-additional-fee='true'">
			deposit               = new Object();
			...
			deposit.minAmount     = '<xsl:value-of select="@min-amount"/>'
	...
	<xsl:template name="viewHtml" mode="view" match="form-data">
		...
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Вклад для пополнения</xsl:with-param>
			<xsl:with-param name="rowValue">
				<xsl:value-of select="depositAccount"/>&nbsp;<xsl:value-of select="depositDescription"/>
			</xsl:with-param>
		</xsl:call-template>
		<xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">Номер договора</xsl:with-param>
