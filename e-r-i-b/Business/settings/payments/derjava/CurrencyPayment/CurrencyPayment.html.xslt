...
<!-- Функции setConditions() не было в этой xslt. Наверное, забыли реализовать. Для Демо Стенда закоментил -->
    ...
    <xsl:template match="/form-data" mode="view">
	    ...
	    <xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">SWIFT</xsl:with-param>
			<xsl:with-param name="rowValue"><xsl:value-of select="receiverBankSWIFT"/></xsl:with-param>
		</xsl:call-template>
	    ...
	    <xsl:call-template name="titleRow">
	     ...
	     <xsl:call-template name="standartRow">
			<xsl:with-param name="rowName">SWIFT</xsl:with-param>
			<xsl:with-param name="rowValue"><xsl:value-of select="intermediaryBankSWIFT"/></xsl:with-param>
		</xsl:call-template>
