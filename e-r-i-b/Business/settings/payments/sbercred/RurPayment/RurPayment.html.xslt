...
<script type="text/javascript">
	...
	function selectNDS(nullNDS)
	{
		...
		else
		{
			...
			document.getElementById("NDS").value = 18;
...
<tr>
	<td class="Width120 LabelAll" colspan="2" style="text-align:left;">&nbsp;<b>Размер НДС</b>
	</td>
</tr>
<tr>
...
<script type="text/javascript">
	function openSelectReceiver()
	{
		window.open(document.webRootPrivate + '/receivers/list.do?action=getReceiverInfo',
				'Receivers', "fullscreen=1,resizable=1,menubar=0,toolbar=0,scrollbars=1");
	}
	...
	function setBankInfo(Info)
	{
		...
		//			 setElement("ground",Info["ground"]);
			...
			var ndsValue = document.getElementById("ndsValue");
			if (nds.value == null || nds.value == '')
			{
				nds.value = 18;
				calculateNDS();
			}
			selectTypePerson("<xsl:value-of select="receiverType"/>");
