...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/form-data">
		<payment>
			...
			<ground>
				<xsl:if test="chargeNDS='no'"><xsl:value-of select="NDSText"/>. НДС не облагается</xsl:if><xsl:if test="chargeNDS='yes'"><xsl:value-of select="NDSText"/>, в т.ч. НДС <xsl:value-of select="NDS"/>% : <xsl:value-of select="NDSSum"/> р.</xsl:if>
			</ground>
