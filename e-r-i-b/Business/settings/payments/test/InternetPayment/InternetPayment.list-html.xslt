...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="tableRow" match="/list-data/entity-list/entity">
		<tr>
			...
			<td class="listItem">
				&nbsp;<xsl:choose>
					<xsl:when test="attributes.internetProvider.value='Stream'">МТУ-Интел</xsl:when>
					<xsl:when test="attributes.internetProvider.value='Corbina'">Корбина-Телеком</xsl:when>
                    <xsl:when test="attributes.internetProvider.value='Zebra'">Зебра-Телеком</xsl:when>
                    <xsl:when test="attributes.internetProvider.value='ROL'">РОЛ</xsl:when>
                </xsl:choose>&nbsp;
			</td>
			<td class="listItem">
