	    ...
	    <table cellspacing="0" cellpadding="2" class="userTab MaxSize">
		    ...
		    <xsl:call-template name="out-row">
			    ...
			    <xsl:with-param name="value1">
				    ...
				    <xsl:if test="string(number($initInstalmentsString))!='NaN'">
				        <xsl:value-of select="number($initInstalmentsString) * 100"/>%
				    </xsl:if>
			    </xsl:with-param>
			    <xsl:with-param name="value2">
				    <xsl:if test="$initInstalmentsString != ''">от </xsl:if>
		    ...
		    <xsl:variable name="thirdDelayPenalty">
			...
			<xsl:call-template name="out-row">
			    <xsl:with-param
					    name="description">Штраф за любой последующий пропуск очередного платежа</xsl:with-param>
				   <xsl:with-param name="value1" select="$thirdDelayPenalty"/>
				   <xsl:with-param name="value2" select="$thirdDelayPenalty"/>
			    <xsl:with-param name="hint">
		    ...
		    <xsl:call-template name="out-row">
			    ...
			    <xsl:with-param name="hint">Обязательное страхование предмета залога (транспортного средства) в соответствии с требованиями Банка</xsl:with-param>
			</xsl:call-template>

		    <xsl:call-template name="out-row">
			    <xsl:with-param name="description">Штраф за непредоставление ПТС (сумма в день)</xsl:with-param>
				<xsl:with-param name="value1">
					<xsl:call-template name="getValues">
					   <xsl:with-param name="valueName">ptsPenalty</xsl:with-param>
					   <xsl:with-param name="type">currencies</xsl:with-param>
				   </xsl:call-template>
				</xsl:with-param>
			    <xsl:with-param name="value2">
			    ...
			    <xsl:with-param name="hint">При продаже транспортного средства с использованием Кредита обязательным условием является передача Заемщиком в Банк на хранение оригинала ПТС</xsl:with-param>
			</xsl:call-template>
