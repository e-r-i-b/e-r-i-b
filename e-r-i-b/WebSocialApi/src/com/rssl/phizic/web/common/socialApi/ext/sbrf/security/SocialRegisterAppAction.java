...
import com.rssl.auth.csa.wsclient.exceptions.SendSmsMessageException;
 ...
 * @ $Author$
 * @ $Revision$
 * Регистрация социального приложения
 */

public class SocialRegisterAppAction extends SocialLoginNamePasswordAction
{
	...
	private static final String CAPTCHA_CODE_SESSION_KEY = "social_registration_captcha_code_key"; //ключ атрибута сессии, хранящего код капчи
	...
	public ActionForward register(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			if (mobilePlatformService.isRegistrationUseCaptcha(appType))
			{
    ...
    {
       return RegisterAppForm.START_SOCIAL_REGISTRATION_FORM;
    }

    protected Form getMobileRegisterCheckCapchaForm()
    {
       return RegisterAppForm.CHECK_CAPTCHA_SOCIAL_REGISTRATION_FORM;
    }

	protected ActionForward getSMSErrorForward(ActionMapping mapping, HttpServletRequest request, String phones, Throwable e)
	{
		log.error("При регистрации в социальном приложении по номеру телефона " + phones + " обнаружена смена SIM-карты", e);
	...
	public ActionForward confirm(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		FormProcessor<ActionMessages, ?> formProcessor =
				FormHelper.newInstance(new RequestValuesSource(currentRequest()), RegisterAppForm.CONFIRM_SOCIAL_REGISTRATION_FORM);
		...
		try
		{
			...
			new ConfirmSocialRegistrationOperation(result);
		}
		catch (ResetMobileGUIDException e)
		{
			log.error("Неверная авторизация. Неактивный mGUID.", e);
	...
	{
		return RegisterAppForm.SET_PIN_SOCIAL_REGISTRATION_FORM;
	}
	...
	{
		return new SocialAppFinishRegistrationOperation(data);
	}
	...
	{
		RegisterAppForm frm = (RegisterAppForm) form;
        ShowCaptchaSocialRegistrationOperation operation = new ShowCaptchaSocialRegistrationOperation(result);
	...
	{
		StartSocialRegistrationOperation operation = new StartSocialRegistrationOperation(result);
