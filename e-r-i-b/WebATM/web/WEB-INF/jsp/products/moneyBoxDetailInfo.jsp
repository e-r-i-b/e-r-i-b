<%--
  Пользователь: usachev
  Дата: 27.04.15
  Описание: Страница для вывода детальной информации по копилкам
--%>
...
<c:set var="formWhithMoneyBox" value="${phiz:currentForm(pageContext)}"/>
<moneyBoxes>
   <c:forEach var="moneyBox" items="${formWhithMoneyBox.moneyBoxes}">
       <moneyBox>
                   ...
                   <c:when test="${typeSum == 'FIXED_SUMMA'}">
                       ...
                       <c:choose>
                           <c:when test="${period == 'ONCE_IN_WEEK'}">
                               <bean:message bundle="${bundle}" key="template.description.period.dayOfWeek.by"/>
                               ${phiz:dayNumberToString(longOfferPayDay.weekDay)}
                               <bean:message bundle="${bundle}" key="template.description.typeSum.FIXED_SUMMA"/>
                           </c:when>
                           <c:when test="${period == 'ONCE_IN_MONTH'}">
                               <c:out value="${longOfferPayDay.day}"/><bean:message bundle="${bundle}" key="template.description.period.dayOfMonth"/><bean:message
                                   bundle="${bundle}"
                                   key="template.description.typeSum.FIXED_SUMMA"/>
                           </c:when>
                           <c:when test="${period == 'ONCE_IN_QUARTER'}">
                               ...
                               <c:out value="${longOfferPayDay.monthInQuarter}"/><bean:message bundle="${bundle}" key="template.description.period.monthOfQuarter"/><bean:message
                                   bundle="${bundle}" key="template.description.typeSum.FIXED_SUMMA"/>
                           </c:when>
                           <c:when test="${period == 'ONCE_IN_YEAR'}">
                               <c:out value="${longOfferPayDay.day}"/>
                               ${phiz:monthNumberToString(longOfferPayDay.monthInYear)}
                               <bean:message bundle="${bundle}" key="template.description.typeSum.FIXED_SUMMA"/>
                           </c:when>
                       </c:choose>
                       ${cardName}
                   ...
                   <c:when test="${typeSum == 'PERCENT_BY_ANY_RECEIPT' || typeSum == 'PERCENT_BY_DEBIT'}">
                       ...
                       <c:choose>
                           <c:when test="${not empty amountValue}">
                               ${cardName}
                               <c:out value="${phiz:getCutCardNumber(cardNumber)}"/> <bean:message bundle="${bundle}" key="template.description.maxSum"/>
                               <fmt:formatNumber value="${amountValue.decimal}" pattern="0.00"/>
                               <c:out value="${amountValue.currency.code}"/>
                           </c:when>
                           <c:otherwise>
           ...
           <executionEventDescription>${description}</executionEventDescription>
