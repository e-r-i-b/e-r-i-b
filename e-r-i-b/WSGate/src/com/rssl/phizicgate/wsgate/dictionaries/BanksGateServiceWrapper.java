 ...
 * @ $Revision$
 */
public class BanksGateServiceWrapper  extends JAXRPCClientSideServiceBase<GateService_Stub> implements BanksGateService
{
	public BanksGateServiceWrapper(GateFactory factory) throws GateException
	{
		super(factory);
		String url = factory.config(WSGateConfig.class).getURL() + "/BanksGateServiceImpl";
		GateServiceImpl_Impl service = new GateServiceImpl_Impl();
		initStub((GateService_Stub) service.getGateServicePort(), url);
	}

	public List<Bank> getAll(int firstResult, int maxResults) throws GateException, GateLogicException
	{
		try
		{          
			List<com.rssl.phizicgate.wsgate.dictionaries.generated.Bank>  generatedBanks =  getStub().getAll(firstResult, maxResults);
			List<Bank> banks = new ArrayList<Bank>();

			BeanHelper.copyPropertiesWithDifferentTypes(banks, generatedBanks, TypesCorrelation.types);
