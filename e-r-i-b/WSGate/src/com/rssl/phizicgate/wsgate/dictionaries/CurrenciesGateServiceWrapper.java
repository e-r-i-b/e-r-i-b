 ...
 * @ $Revision$
 */
public class CurrenciesGateServiceWrapper  extends JAXRPCClientSideServiceBase<GateService_Stub> implements CurrenciesGateService
{
	public CurrenciesGateServiceWrapper(GateFactory factory) throws GateException
	{
		super(factory);
		String url = factory.config(WSGateConfig.class).getURL() + "/CurrenciesGateServiceImpl";
		GateServiceImpl_Impl service = new GateServiceImpl_Impl();
		initStub((GateService_Stub) service.getGateServicePort(), url);
	}

	public List<Currency> getAll(int firstResult, int maxResults) throws GateException, GateLogicException
	{
		try
		{
			List<com.rssl.phizicgate.wsgate.dictionaries.generated.Currency>  generatedCurrencies =  getStub().getAll(firstResult, maxResults);
			List<Currency> currencies = new ArrayList<Currency>();

			BeanHelper.copyPropertiesWithDifferentTypes(currencies, generatedCurrencies, TypesCorrelation.types);
